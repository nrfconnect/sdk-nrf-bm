...
* Support for the nRF54LS05A SoC, emulated on the nRF54LS05 DK through the ``bm_nrf54ls05dk/nrf54ls05a/cpuapp/*`` board targets.
* Support for the nRF54LC10A SoC, running on the nRF54LC10 DK (PCA10226)
* ``BOARD_EXTERNAL_MEMORY_*`` macros to the :file:`board-config.h` file of the ``bm_nrf54l15dk`` and ``bm_nrf54lm20dk`` board targets  (SPIM instance, SCK/MOSI/MISO/CS, and WP#/RST# strap pins) for on-board SPI external flash.
