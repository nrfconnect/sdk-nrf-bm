...
.. note::
   The clock source Kconfig options below must match the oscillator hardware available on your board, as described in its Devicetree.
...
The following Kconfig options select whether the SoftDevice's low-frequency clock is sourced from an external crystal oscillator (LFXO) or the internal RC oscillator (LFRC).
...
When using the internal RC oscillator as the low-frequency clock source, the following Kconfig options control how often and at what interval the clock is calibrated:
...
* :kconfig:option:`CONFIG_NRF_SDH_CLOCK_LF_RC_TEMP_CTIV` - How often the RC oscillator is calibrated if the temperature has not changed, in number of calibration intervals (range 0-33, default 2).

  * ``0``: Always calibrate even if the temperature hasn't changed.
  * ``1``: Invalid.
  * ``2`` - ``33``: Check the temperature and only calibrate if it has changed. However, always calibrate when this many intervals have passed.

Clock accuracy
--------------

The accuracy, in ppm, of the low-frequency clock source must be set to one of the following Kconfig options to match the accuracy of the clock source.
The SoftDevice uses this value to size its Bluetooth LE connection timing windows. A less accurate clock requires wider windows to avoid missing connection events, at the cost of additional radio-on time.
...
The high-frequency clock (HFCLK) can be sourced from either the internal oscillator (HFINT) or the crystal oscillator (HFXO).
HFXO must be running to achieve the clock accuracy the radio requires.
If HFXO is not already running, the SoftDevice starts it before radio activity.
...
The SoftDevice handler builds the configuration from the following Kconfig options and associates it with the tag defined by the :kconfig:option:`CONFIG_NRF_SDH_BLE_CONN_TAG` Kconfig option.

These Kconfig options are only available when the :kconfig:option:`CONFIG_NRF_SDH_BLE` Kconfig option is enabled (the default).
...
You can configure the Bluetooth LE stack using the following Kconfig options:
...
These Kconfig options are interrelated. The number of concurrent peripheral and/or central connections is bounded by the total connection count, and the maximum depends on the SoftDevice variant in use.
...
Larger values generally increase throughput or capacity at the cost of additional RAM and radio-on time.
...
* :kconfig:option:`CONFIG_NRF_SDH_BLE_GAP_EVENT_LENGTH` - Time reserved for each connection event, in 1.25 ms units (range 2-3200, default 3).
  ...
  A longer event length allows more data per interval but keeps the radio on longer.
...
* :kconfig:option:`CONFIG_NRF_SDH_BLE_SERVICE_CHANGED` - Include the Service Changed characteristic in the Attribute Table.
  ...
  Use :c:func:`sd_ble_gatts_service_changed` to send a Service Changed indication, documented in the :ref:`api_softdevice`.
...
* :kconfig:option:`CONFIG_NRF_SDH_STR_TABLES` - Build string tables so that :c:func:`nrf_sdh_ble_evt_to_str` and :c:func:`nrf_sdh_soc_evt_to_str` can return human-readable event names (enabled by default).
...
The Kconfig options controlling this output are described in :ref:`nrf_sdh_logging_diagnostics` above.
...
The SoftDevice handler uses the standard logging module, so you can control how much it logs with the :kconfig:option:`CONFIG_NRF_SDH_LOG_LEVEL` Kconfig option.
