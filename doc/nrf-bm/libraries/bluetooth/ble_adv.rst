...
The Advertising Module handles the BLE advertising of your application. You can use it for many typical connectable advertising scenarios.
If your application require non-connectable advertising the module can be taken as a reference together with the :ref:`api_ble_adv_data_encoder` API.

Overview
********

The module covers the most common advertising behavior and can be adapted by enabling or disabling specific advertising modes.
...
The library support the following connectable advertising modes:
...
* ``Slow`` - Advertise to surrounding devices with a longer advertising interval than in fast advertising mode.
  This advertising mode conserves power and causes less traffic for other wireless devices that might be present.
...
If you start advertising in direct mode, the module first attempts ``Direct`` advertising.
If no information about a previous connection is available or the previous peer is not available, the module starts ``Fast`` advertising.
...
You can configure the time-out thresholds and advertising intervals for ``Direct``, ``Fast``, and ``Slow`` advertising.
...
| Header file: :file:`include/ble_adv.h`
| Source files: :file:`lib/ble_adv/`

:ref:`Bluetooth LE Advertising library API reference <api_ble_adv>`

| Header file: :file:`include/ble_adv_data.h`
| Source files: :file:`lib/ble_adv/`
