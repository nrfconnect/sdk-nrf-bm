...
You can use this kind of record in a message together with other records to provide extra information.
The Text Records library provides functions for creating and encoding text records.

Text records contain a payload (the actual text string) and a language identifier.
A message can contain only one text record for each language code, so you cannot add two different text records with language code "en" to the same message, for example.

The following code example shows how to generate a raw text message that contains a text record.

1. Generate the text record.
