...
static uint8_t lbu_buffer[CONFIG_LOG_BACKEND_LITE_UARTE_BUFFER_SIZE];
static uint32_t log_format_current = CONFIG_LOG_BACKEND_LITE_UARTE_OUTPUT_DEFAULT;
...
static int uarte_init(void)
{
	...
	if (IS_ENABLED(CONFIG_LOG_BACKEND_LITE_UARTE_USE_HWFC)) {
	...
	if (IS_ENABLED(CONFIG_LOG_BACKEND_LITE_UARTE_PARITY_INCLUDED)) {
		uarte_config.config.parity = NRF_UARTE_PARITY_INCLUDED;
	}

	uarte_config.interrupt_priority = CONFIG_LOG_BACKEND_LITE_UARTE_IRQ_PRIO;

	/** We need to connect the IRQ ourselves. */
	IRQ_CONNECT(NRFX_IRQ_NUMBER_GET(
		NRF_UARTE_INST_GET(LITE_UARTE_CONSOLE_UARTE_INST)),
		CONFIG_LOG_BACKEND_LITE_UARTE_IRQ_PRIO,
...
LOG_OUTPUT_DEFINE(lbu_output, char_out, lbu_buffer, CONFIG_LOG_BACKEND_LITE_UARTE_BUFFER_SIZE);
...
static void process(const struct log_backend *const backend, union log_msg_generic *msg)
{
	...
	log_output_func(&lbu_output, &msg->log, flags);
}
...
{
	log_backend_std_dropped(&lbu_output, cnt);
}
...
LOG_BACKEND_DEFINE(log_backend_lite_uarte, log_backend_api, AUTO_START, NULL);
