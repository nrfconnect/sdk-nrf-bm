...
static uint32_t write_execute(const struct bm_storage_sd_op *op)
{
	...
	len = MIN(op->write.len - op->write.offset, CONFIG_BM_STORAGE_BACKEND_SD_MAX_WRITE_SIZE);
	...
	if (len < pad_unit) {
		__ASSERT_NO_MSG(op->storage->flags.is_write_padded);

		/* Copy the remaining bytes into the padding buffer */
		memcpy(pad_buffer, src, len);
		/* Pad it with what's already stored in memory */
		memcpy(pad_buffer + len, (const uint8_t *)dest + len, pad_unit - len);
	...
	len = MAX(1, len / SD_PROGRAM_UNIT_BYTES);

	return sd_flash_write(dest, src, len);
}
