 ...
 * +-----------+----------+----------+-----+----------+
 *
 * Implementation is designed to cover only-write operations or only-read operations at once.
 *
 * How to write data to the clipboard:
 * 1. bm_rmem_writer_init() to initialize the clipboard writer context.
 ...
 * 1. bm_rmem_reader_init() to initialize the clipboard reader context, verifies the content
 *    integrity.
 * 3. bm_rmem_data_get() to read data from the clipboard. Multiple data TLVs can be read.
 * 4. bm_rmem_clear() optionaly to clear the clipboard content so it will be illegibel for
 *    the next user.
...
LOG_MODULE_REGISTER(bm_rmem_clipboard, CONFIG_BM_RMEM_CLIPBOARD_LOG_LEVEL);
