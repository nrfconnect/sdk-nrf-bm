			    ...
			    struct img_mgmt_upload_action *action)
{
	...
	if (req->off == 0) {
		 ...
		 */
		if ((req->data_sha.len > 0) && (img_mgmt_state.area_id != -1)) {
			if ((img_mgmt_state.data_sha_len == req->data_sha.len) &&
			    !memcmp(img_mgmt_state.data_sha, req->data_sha.value,
				    req->data_sha.len)) {
		...
		action->area_id = img_mgmt_state.area_id;
		action->size = img_mgmt_state.size;

		if (req->off != img_mgmt_state.off) {
			/*
		...
		if (req->img_data.len > action->size ||
		    (action->size - req->img_data.len) < req->off) {
