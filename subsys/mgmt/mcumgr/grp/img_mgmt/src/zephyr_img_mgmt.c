...
#else
static struct flash_img_context ctx;
#if defined(CONFIG_MCUMGR_GRP_IMG_USE_BUFFERED_WRITES) 
#include <bm/storage/bm_storage.h>

uint8_t tail;
uint8_t head = 0;
uint8_t lastsize;
uint8_t pktbuf[512] __attribute__ ((aligned (4)));
static void bm_storage_evt_handler_a(struct bm_storage_evt *evt)
{
			LOG_ERR("storage event tail %d", tail);
	if(tail){
		for(int i=0; i<tail; i++){
			pktbuf[i]=pktbuf[i+lastsize];
		}
		head=tail;
	}else{
		head=0;
			      ...
			      bool last)
{
	...
	if (offset == 0) {
		...
		struct bm_storage_config storage_config_a = {
        	...
        	bm_storage_init(&storage_a, &storage_config_a);
	}
	memcpy(pktbuf+head, data, num_bytes);
	tail = ((num_bytes + head)) % storage_a.nvm_info->program_unit;
	int write_sz = num_bytes + head;
#if defined(CONFIG_MCUMGR_GRP_IMG_USE_BUFFERED_WRITES) 
	if(last){
		int pad = storage_a.nvm_info->program_unit - tail;
		LOG_ERR("last adding %d num %d", pad, num_bytes);
		for(int i = 0; i<pad; i++){
			pktbuf[write_sz+i] = 0xFF;
		}
		write_sz += pad;
	}else{
		write_sz -= tail;
	}
	LOG_ERR("numbytes %d tail %d  head %d write_sz %d unit %d last %d",num_bytes, tail, head, write_sz, storage_a.nvm_info->program_unit, last);
        int err = bm_storage_write(&storage_a, storage_a.start_addr + write_offset, pktbuf, write_sz, NULL);
	write_offset += write_sz;
        if (err) {
