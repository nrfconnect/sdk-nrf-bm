...
struct img_mgmt_state img_mgmt_state;
...
{
	memset(&img_mgmt_state, 0, sizeof(img_mgmt_state));
	img_mgmt_state.area_id = -1;
}
...
img_mgmt_upload_good_rsp(struct smp_streamer *ctxt)
{
	...
	ok = ok && zcbor_tstr_put_lit(zse, "off")		&&
		   zcbor_size_put(zse, img_mgmt_state.off);
...
img_mgmt_upload(struct smp_streamer *ctxt)
{
	...
	img_mgmt_state.area_id = action.area_id;
	img_mgmt_state.size = action.size;
	...
	if (req.off == 0) {
		 ...
		 * resuming upload.
		 */
		img_mgmt_state.data_sha_len = req.data_sha.len;
		memcpy(img_mgmt_state.data_sha, req.data_sha.value, req.data_sha.len);
		memset(&img_mgmt_state.data_sha[req.data_sha.len], 0,
	...
	if (req.img_data.len != 0) {
		...
		if (img_mgmt_state.off + req.img_data.len == img_mgmt_state.size) {
			last = true;
		}
		...
		if (rc == 0 || rc == MGMT_ERR_EBUSY) {
			if (rc == 0) {
				img_mgmt_state.off += action.write_bytes;
			} else {
		...
		if (img_mgmt_state.off == img_mgmt_state.size) {
			/* Done */
			reset = true;
		}
	}
