...
LOG_MODULE_REGISTER(bm_sdh, CONFIG_BM_SDH_LOG_LEVEL);

#if ((CONFIG_BM_SDH_CLOCK_LF_SRC == NRF_CLOCK_LF_SRC_RC) &&                                       \
	(CONFIG_BM_SDH_CLOCK_LF_ACCURACY != NRF_CLOCK_LF_ACCURACY_500_PPM))
...
static char *req_tostr(enum bm_sdh_state_req r)
{
	switch (r) {
	case BM_SDH_STATE_REQ_ENABLE:
		return "enable";
	case BM_SDH_STATE_REQ_DISABLE:
		return "disable";
	default:
		return "unknown";
	};
}
static char *state_tostr(enum bm_sdh_state_evt s)
{
	switch (s) {
	case BM_SDH_STATE_EVT_ENABLE_PREPARE:
		return "enabling";
	case BM_SDH_STATE_EVT_ENABLED:
		return "enabled";
	case BM_SDH_STATE_EVT_DISABLE_PREPARE:
		return "disabling";
	case BM_SDH_STATE_EVT_DISABLED:
		return "disabled";
	default:
		return "unknown";
	};
...
static int sdh_state_req_observer_notify(enum bm_sdh_state_req req)
{
	...
	TYPE_SECTION_FOREACH(struct bm_sdh_state_req_observer, bm_sdh_state_req_observers, obs) {
...
static void sdh_state_evt_observer_notify(enum bm_sdh_state_evt state)
{
	if (IS_ENABLED(CONFIG_BM_SDH_STR_TABLES)) {
...
int bm_sdh_enable_request(void)
{
	...
	const nrf_clock_lf_cfg_t clock_lf_cfg = {
		.source = CONFIG_BM_SDH_CLOCK_LF_SRC,
		.rc_ctiv = CONFIG_BM_SDH_CLOCK_LF_RC_CTIV,
		.rc_temp_ctiv = CONFIG_BM_SDH_CLOCK_LF_RC_TEMP_CTIV,
		.accuracy = CONFIG_BM_SDH_CLOCK_LF_ACCURACY
	};
	...
	err = sdh_state_req_observer_notify(BM_SDH_STATE_REQ_ENABLE);
	if (err) {
	...
	sdh_state_evt_observer_notify(BM_SDH_STATE_EVT_ENABLE_PREPARE);
	...
	sdh_state_evt_observer_notify(BM_SDH_STATE_EVT_ENABLED);
...
int bm_sdh_disable_request(void)
{
	...
	err = sdh_state_req_observer_notify(BM_SDH_STATE_REQ_DISABLE);
	if (err) {
		/** TODO: should this be Success instead? */
		/* Leave sdh_transition to 1, so process can be continued */
		__ASSERT(err == -EBUSY, "Unknown return value %d from sdh req observer", err);
	...
	sdh_state_evt_observer_notify(BM_SDH_STATE_EVT_DISABLE_PREPARE);
	...
	/* Notify observers about a finished SoftDevice enable process. */
	sdh_state_evt_observer_notify(BM_SDH_STATE_EVT_DISABLED);
...
int bm_sdh_request_continue(void)
{
	...
	if (sdh_enabled) {
		return bm_sdh_disable_request();
	} else {
		return bm_sdh_enable_request();
	}
}
...
void bm_sdh_evts_poll(void)
{
	...
	TYPE_SECTION_FOREACH(struct bm_sdh_stack_evt_observer, bm_sdh_stack_evt_observers, obs) {
		obs->handler(obs->context);
	}
}

#if defined(CONFIG_BM_SDH_DISPATCH_MODEL_IRQ)
...
#elif defined(CONFIG_BM_SDH_DISPATCH_MODEL_SCHED)
...
#elif defined(CONFIG_BM_SDH_DISPATCH_MODEL_POLL)

#endif /* BM_SDH_DISPATCH_MODEL */
