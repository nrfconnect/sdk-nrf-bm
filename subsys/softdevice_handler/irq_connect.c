...
extern void CRACEN_IRQHandler(void);
...
#define PRIO_LOW 4	/* SoftDevice low priority interrupt */
	...
	IRQ_DIRECT_CONNECT(CRACEN_IRQn, PRIO_LOW, CRACEN_IRQHandler, IRQ_ZERO_LATENCY);
	 ...
	 * level to 0 (PRIO_HIGH).
	 * Restore the priority level to PRIO_LOW for these specific interrupts.
	 */
	NVIC_SetPriority(CLOCK_POWER_IRQn, PRIO_LOW);
	NVIC_SetPriority(CRACEN_IRQn, PRIO_LOW);
	NVIC_SetPriority(ECB00_IRQn, PRIO_LOW);
	NVIC_SetPriority(AAR00_CCM00_IRQn, PRIO_LOW);
	NVIC_SetPriority(SWI00_IRQn, PRIO_LOW);
