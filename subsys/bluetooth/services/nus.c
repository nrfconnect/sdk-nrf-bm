...
static void on_connect(struct ble_nus *nus, ble_evt_t const *ble_evt)
{
	...
	err = sd_ble_gatts_value_get(ble_evt->evt.gap_evt.conn_handle,
				     ...
				     &gatts_val);
	if ((err == 0) && (nus->event_handler != NULL) &&
...
static void on_hvx_tx_complete(struct ble_nus *nus, ble_evt_t const *ble_evt)
{
	...
	if ((ctx->is_notification_enabled) && (nus->event_handler != NULL)) {
		evt.link_ctx = ctx;
		nus->event_handler(&evt);
	}
}
...
int ble_nus_init(struct ble_nus *nus, struct ble_nus_config const *cfg)
{
	...
	nus->event_handler = cfg->event_handler;
