 ...
 */
static int ctrlpt_char_add(struct ble_bms *bms, struct ble_bms_config const *bms_config)
{
	int err;
...
{
	struct ble_bms_features *feature = &bms->feature;
 ...
 */
static int ctrlpt_decode(uint8_t const *rcvd_val, uint16_t len,	struct ble_bms_ctrlpt *ctrlpt)
{
	uint16_t pos = 0;
 ...
 * @return 0 on success, otherwise an error code returned by @ref characteristic_add.
 */
static int feature_char_add(struct ble_bms *bms, struct ble_bms_config const *bms_config)
{
 ...
 */
uint16_t on_qwr_auth_req(struct ble_bms *bms, struct ble_qwr *qwr,
			 const struct ble_qwr_evt *evt)
{
	...
	err = ble_qwr_value_get(qwr, evt->auth_req.attr_handle, mem_buffer, &len);
	if (err) {
		LOG_ERR("Control point write: Operation failed.");
		return BLE_BMS_OPERATION_FAILED;
	}
 ...
 * @retval BLE_BMS_OPCODE_NOT_SUPPORTED If the received opcode is not supported.
 */
uint16_t on_qwr_exec_write(struct ble_bms *bms, struct ble_qwr *qwr,
			   const struct ble_qwr_evt *evt)
{
	int err;
	uint16_t len = BLE_BMS_CTRLPT_MAX_LEN;
...
int ble_bms_set_conn_handle(struct ble_bms *bms, uint16_t conn_handle)
{
	if (!bms) {
		return NRF_ERROR_NULL;
	}
...
int ble_bms_init(struct ble_bms *bms, struct ble_bms_config *bms_config)
{
	int err;
	ble_uuid_t ble_uuid;
	...
	if ((bms_config->qwr != NULL) && (bms_config->qwr_count == 0)) {
		err = ble_qwr_attr_register(bms_config->qwr, bms->ctrlpt_handles.value_handle);
		if (err) {
			return err;
		}
	} else if (bms_config->qwr && (bms_config->qwr_count > 0)) {
		for (uint32_t i = 0; i < bms_config->qwr_count; i++) {
			err = ble_qwr_attr_register(&bms_config->qwr[i],
...
int ble_bms_auth_response(struct ble_bms *bms, bool authorize)
{
	if (!bms) {
		return NRF_ERROR_NULL;
	}
