...
static void on_write(struct ble_bas *bas, const ble_gatts_evt_t *gatts_evt)
{
	...
	LOG_DBG("Battery level notifications %sabled for peer %#x",
	...
	struct ble_bas *bas, uint16_t conn_handle, uint8_t battery_level)
{
	...
	/* No CCCD descriptor exists when notifications are not supported. Skip the read. */
	if (!bas->can_notify) {
		notify = false;
	} else {
		...
		nrf_err = sd_ble_gatts_value_get(conn_handle,
						 bas->battery_level_handles.cccd_handle, &val);
	...
	if (notify) {
		...
		hvx.p_len  = &(uint16_t){sizeof(battery_level)};
		hvx.p_data = &battery_level;
		...
		/* Update battery level in the GATT database without notifying connected peer */
	...
	/* Cache the new battery level locally */
