			       ...
			       struct ble_hids_input_report *report)
{
	...
	/* Pointer into the per-connection client context where the input report data is stored */
	...
	if (notify) {
		...
		/* Update HID report in the GATT database without notifying connected peer */
		ble_gatts_value_t gatts_value = {0};

		gatts_value.len = report->len;
		gatts_value.p_value = report->data;

		nrf_err = sd_ble_gatts_value_set(BLE_CONN_HANDLE_INVALID,
						 rep_char->char_handles.value_handle, &gatts_value);
		if (nrf_err) {
			LOG_ERR("Failed to update report, nrf_error %#x", nrf_err);
			return nrf_err;
		}
	}
				       ...
				       struct ble_hids_boot_keyboard_input_report *report)
{
	...
	if (notify) {
		...
		/* Update boot keyboard report in GATT database without notifying peer */
		ble_gatts_value_t gatts_value = {0};
		...
		nrf_err = sd_ble_gatts_value_set(BLE_CONN_HANDLE_INVALID,
						 hids->boot_kb_inp_rep_handles.value_handle,
						 &gatts_value);
		if (nrf_err) {
			LOG_ERR("Failed to update boot keyboard report, nrf_error %#x", nrf_err);
			return nrf_err;
		}
	}
					  ...
					  struct ble_hids_boot_mouse_input_report *report)
{
	...
	if (notify) {
		...
		/* Update boot mouse report in the GATT database without notifying connected peer */
		ble_gatts_value_t gatts_value = {0};

		gatts_value.len = total_len;
		...
		nrf_err = sd_ble_gatts_value_set(BLE_CONN_HANDLE_INVALID,
						 ...
						 &gatts_value);
		if (nrf_err) {
			LOG_ERR("Failed to update boot mouse report, nrf_error %#x", nrf_err);
			return nrf_err;
		}
	}
