			       ...
			       struct ble_hids_input_report *report)
{
	...
	nrf_err = sd_ble_gatts_value_get(conn_handle, rep_char->char_handles.cccd_handle,
					 &cccd_val);
	if (nrf_err) {
	...
	if (*hvx_params.p_len != report->len) {
		/*
		 * sd_ble_gatts_hvx updates *p_len to the number of bytes actually written.
		 * This can be less than report->len when the negotiated ATT MTU is too small to
		 * fit the full report in a single notification.
		 */
		LOG_ERR("Report truncated: wrote %u of %u bytes", *hvx_params.p_len, report->len);
		return NRF_ERROR_DATA_SIZE;
	}
				       ...
				       struct ble_hids_boot_keyboard_input_report *report)
{
	...
	/* Store the new report data in the hosts context. */
	...
	/* Send the boot keyboard report as a notification. */
	ble_gatts_hvx_params_t hvx_params = {
	...
	if (*hvx_params.p_len != report->len) {
		 ...
		 * fit the full report in a single notification.
		 */
		LOG_ERR("Boot keyboard report truncated: wrote %u of %u bytes",
			*hvx_params.p_len, report->len);
		return NRF_ERROR_DATA_SIZE;
	}
					  ...
					  struct ble_hids_boot_mouse_input_report *report)
{
	...
	/* Send the boot mouse report as a notification. */
	ble_gatts_hvx_params_t hvx_params = {
	...
	if (*hvx_params.p_len != total_len) {
		/*
		 * sd_ble_gatts_hvx updates *p_len to the number of bytes actually written.
		 * This can be less than total_len when the negotiated ATT MTU is too small to
		 * fit the full report in a single notification.
		 */
		LOG_ERR("Boot mouse report truncated: wrote %u of %u bytes",
			*hvx_params.p_len, total_len);
		return NRF_ERROR_DATA_SIZE;
	}
