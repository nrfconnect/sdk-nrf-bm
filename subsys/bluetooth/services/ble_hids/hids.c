...
static void on_connect(struct ble_hids *hids, ble_evt_t const *ble_evt)
{
	...
	if (nrf_err) {
		...
		LOG_ERR("Failed to get link context, nrf_error %d", nrf_err);
	...
	if (hids->protocol_mode_handles.value_handle) {
		...
		if (nrf_err) {
			...
			LOG_ERR("Failed to set GATTS value, nrf_error %d", nrf_err);
			hids->evt_handler(hids, &evt);
		}
	}
}
...
void on_protocol_mode_read_auth(struct ble_hids *hids, ble_evt_t const *ble_evt)
{
	...
	if (host != NULL) {
		...
		if (nrf_err) {
			...
			LOG_ERR("Failed to autorize rw request, nrf_error %d", nrf_err);
			hids->evt_handler(hids, &evt);
		}
	}
}
				      ...
				      uint16_t rep_max_len)
{
	...
	if (host != NULL) {
		...
		if (nrf_err) {
			...
			LOG_ERR("Failed to authorize rw request, nrf_error %d", nrf_err);
			hids->evt_handler(hids, &evt);
		}
	} else {
		return;
			       ...
			       struct ble_hids_input_report *report)
{
	...
	if (report->report_index < hids->input_report_count) {
		...
		if (conn_handle != BLE_CONN_HANDLE_INVALID) {
			...
			if (nrf_err) {
				LOG_ERR("Failed to get link context, nrf_err %d", nrf_err);
				return nrf_err;
			}
			...
			if ((nrf_err == NRF_SUCCESS) && (*hvx_params.p_len != report->len)) {
				LOG_ERR("Failed to update attribute value, nrf_err %#x", nrf_err);
