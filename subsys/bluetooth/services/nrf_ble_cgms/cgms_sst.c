/*
 * Copyright (c) 2016 - 2025 Nordic Semiconductor ASA
 *
 * SPDX-License-Identifier: LicenseRef-Nordic-5-Clause
 */
#include <stdint.h>
#include <string.h>
#include <time.h>
#include <ble.h>
#include <bluetooth/services/ble_cgms.h>
#include <bluetooth/services/uuid.h>
#include "cgms_sst.h"

#include <zephyr/logging/log.h>

LOG_MODULE_DECLARE(ble_cgms, CONFIG_BLE_CGMS_LOG_LEVEL);

void sst_decode(struct ble_cgms_sst *sst, const uint8_t *data, const uint16_t len)
{
	uint32_t index;

	if (len != NRF_BLE_CGMS_SST_LEN) {
		return;
	}

	index = ble_date_time_decode(&sst->date_time, data);

	sst->time_zone = data[index++];
	sst->dst = data[index++];
}

void convert_ble_time_c_time(struct ble_cgms_sst *sst, struct tm *tm)
{
	tm->tm_sec = sst->date_time.seconds;
	tm->tm_min = sst->date_time.minutes;
	tm->tm_hour = sst->date_time.hours;
	tm->tm_mday = sst->date_time.day;
	tm->tm_mon = sst->date_time.month;
	tm->tm_year = sst->date_time.year - 1900;

	/* Ignore daylight saving for this conversion. */
	tm->tm_isdst = 0;
}

void calc_sst(uint16_t offset, struct tm *tm)
{
	/* Time in seconds*/
	time_t time;

	time  = mktime(tm);
	time  = time - (offset * 60);
	*tm = *(localtime(&time));

	if (tm->tm_isdst == 1) {
		/* Daylight saving time is not used and must be removed. */
		tm->tm_hour = tm->tm_hour - 1;
		tm->tm_isdst = 0;
	}
}

static void convert_c_time_ble_time(struct ble_cgms_sst *sst, struct tm *tm)
{
	sst->date_time.seconds = tm->tm_sec;
	sst->date_time.minutes = tm->tm_min;
	sst->date_time.hours = tm->tm_hour;
	sst->date_time.day = tm->tm_mday;
	sst->date_time.month = tm->tm_mon;
	sst->date_time.year = tm->tm_year + 1900;
}

static uint8_t sst_encode(struct ble_cgms_sst *sst, uint8_t *sst_encoded)
{
	uint8_t len;

	len = ble_date_time_encode(&sst->date_time, sst_encoded);

	sst_encoded[len++] = sst->time_zone;
	sst_encoded[len++] = sst->dst;

	return len;
}

static int cgm_update_sst(struct nrf_ble_cgms *cgms, ble_gatts_evt_write_t const *evt_write)
{
	struct ble_cgms_sst sst;
	struct tm      c_time_and_date;

	memset(&sst, 0, sizeof(struct ble_cgms_sst));

	sst_decode(&sst, evt_write->data, evt_write->len);
	convert_ble_time_c_time(&sst, &c_time_and_date);
	calc_sst(cgms->sensor_status.time_offset, &c_time_and_date);
	convert_c_time_ble_time(&sst, &c_time_and_date);

	return cgms_sst_set(cgms, &sst);
}

/**
 * @brief Function for handling the Glucose session start time write event.
 *
 * @param[in] cgms Service instance.
 * @param[in] evt_write WRITE event to be handled.
 */
static void on_sst_value_write(struct nrf_ble_cgms *cgms, ble_gatts_evt_write_t const *evt_write)
{
	uint32_t err;
	ble_gatts_rw_authorize_reply_params_t auth_reply = {
		.type = BLE_GATTS_AUTHORIZE_TYPE_WRITE,
		.params = {
			.write = {
				.gatt_status = BLE_GATT_STATUS_SUCCESS,
				.update = 1,
			},
		},
	};
	struct nrf_ble_cgms_evt cgms_evt = {
		.evt_type = NRF_BLE_CGMS_EVT_ERROR,
	};

	err = sd_ble_gatts_rw_authorize_reply(cgms->conn_handle, &auth_reply);
	if (err != NRF_SUCCESS) {
		if (cgms->evt_handler != NULL) {
			cgms_evt.error.reason = err;
			cgms->evt_handler(cgms, &cgms_evt);
		}
	}

	err = cgm_update_sst(cgms, evt_write);
	if (err != NRF_SUCCESS) {
		if (cgms->evt_handler != NULL) {
			cgms_evt.error.reason = err;
			cgms->evt_handler(cgms, &cgms_evt);
		}
	}
}

/**
 * @brief Function for adding a characteristic for the Session Start Time.
 *
 * @param[in]   cgms   Service instance.
 *
 * @return      NRF_SUCCESS if characteristic was successfully added, otherwise an error code.
 */
int cgms_sst_char_add(struct nrf_ble_cgms *cgms)
{
	uint8_t init_value[NRF_BLE_CGMS_SST_LEN] = {0};

	int err;
	ble_uuid_t char_uuid = {
		.type = BLE_UUID_TYPE_BLE,
		.uuid = BLE_UUID_CGM_SESSION_START_TIME,
	};
	ble_gatts_char_md_t char_md = {
		.char_props = {
			.write = true,
			.read = true,
		},
	};
	ble_gatts_attr_md_t attr_md = {
		.vloc = BLE_GATTS_VLOC_STACK,
		.wr_auth = true,
	};
	ble_gatts_attr_t attr_char_value = {
		.p_uuid = &char_uuid,
		.p_attr_md = &attr_md,
		.p_value = init_value,
		.init_len = NRF_BLE_CGMS_SST_LEN,
		.max_len = NRF_BLE_CGMS_SST_LEN,
	};

	BLE_GAP_CONN_SEC_MODE_SET_OPEN(&attr_md.read_perm);
	BLE_GAP_CONN_SEC_MODE_SET_OPEN(&attr_md.write_perm);

	err = sd_ble_gatts_characteristic_add(cgms->service_handle, &char_md, &attr_char_value,
					      &cgms->char_handles.sst);
	if (err) {
		LOG_ERR("Failed to add GATT CGMS SST characteristic, nrf_error %#x", err);
		return err;
	}

	return NRF_SUCCESS;
}


int cgms_sst_set(struct nrf_ble_cgms *cgms, struct ble_cgms_sst *sst)
{
	uint16_t conn_handle;
	uint16_t value_handle;
	ble_gatts_value_t sst_val;
	uint8_t encoded_start_session_time[NRF_BLE_CGMS_SST_LEN];
	uint8_t gatts_value_set_len = 0;

	gatts_value_set_len = sst_encode(sst, encoded_start_session_time);
	conn_handle = cgms->conn_handle;
	value_handle = cgms->char_handles.sst.value_handle;
	memset(&sst_val, 0, sizeof(ble_gatts_value_t));
	sst_val.len = gatts_value_set_len;
	sst_val.p_value = encoded_start_session_time;
	sst_val.offset  = 0;

	return sd_ble_gatts_value_set(conn_handle, value_handle, &sst_val);
}


void cgms_sst_on_rw_auth_req(struct nrf_ble_cgms *cgms,
			     ble_gatts_evt_rw_authorize_request_t const *auth_req)
{
	if (auth_req->type == BLE_GATTS_AUTHORIZE_TYPE_WRITE) {
		if (auth_req->request.write.handle == cgms->char_handles.sst.value_handle) {
			on_sst_value_write(cgms, &auth_req->request.write);
		}
	}
}
