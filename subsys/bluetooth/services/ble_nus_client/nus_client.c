				   ...
				   struct ble_db_discovery_evt *evt)
{
	...
	const struct ble_gatt_db_char *chars = evt->discovered_db->characteristics;

	/* Check if the NUS was discovered. */
	if ((evt->evt_type == BLE_DB_DISCOVERY_COMPLETE) &&
	    (evt->discovered_db->srv_uuid.uuid == BLE_UUID_NUS_SERVICE) &&
	    (evt->discovered_db->srv_uuid.type == ble_nus_c->uuid_type)) {
		for (uint32_t i = 0; i < evt->discovered_db->char_count; i++) {
			switch (chars[i].characteristic.uuid.uuid) {
			case BLE_UUID_NUS_RX_CHARACTERISTIC:
				nus_c_evt.discovery_complete.nus_rx_handle =
					chars[i].characteristic.handle_value;
				break;

			case BLE_UUID_NUS_TX_CHARACTERISTIC:
				nus_c_evt.discovery_complete.nus_tx_handle =
					chars[i].characteristic.handle_value;
				nus_c_evt.discovery_complete.nus_tx_cccd_handle =
					chars[i].cccd_handle;
				break;
...
static void on_hvx(struct ble_nus_client *ble_nus_c, ble_evt_t const *ble_evt)
{
	struct ble_nus_client_evt ble_nus_c_evt = {
		.evt_type = BLE_NUS_CLIENT_EVT_TX_EVT,
		.tx_data.data = (uint8_t *)ble_evt->evt.gattc_evt.params.hvx.data,
		.tx_data.data_len = ble_evt->evt.gattc_evt.params.hvx.len,
	};
