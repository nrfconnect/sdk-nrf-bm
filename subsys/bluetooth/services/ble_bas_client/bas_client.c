	...
	if (db_evt->evt_type == BLE_DB_DISCOVERY_COMPLETE &&
		...
		for (uint8_t i = 0; i < db_evt->discovered_db->char_count; i++) {
			if (db_evt->discovered_db->characteristics[i].characteristic.uuid.uuid ==
				...
				evt.bas_db.bl_handle = db_evt->discovered_db->characteristics[i]
...
static uint32_t cccd_configure(struct ble_bas_client *ble_bas_client, bool notification_enable)
{
	...
	struct ble_gq_req bas_client_req = {
		...
		.ctx = ble_bas_client,
		.gattc_write.handle = ble_bas_client->peer_bas_db.bl_cccd_handle,
...
uint32_t ble_bas_client_bl_read(struct ble_bas_client *ble_bas_client)
{
	...
	struct ble_gq_req bas_client_req = {
		...
		.gattc_read.handle = ble_bas_client->peer_bas_db.bl_handle,
		.evt_handler = bas_gq_event_handler,
		.ctx = ble_bas_client,
	};
