...
static uint8_t tx_buf[CONFIG_SHELL_BACKEND_BM_UARTE_TX_BUF_SIZE];
			 ...
			 size_t *cnt)
{
	...
	tx_len = MIN(sizeof(tx_buf), length);
	memcpy(tx_buf, data, tx_len);
	*cnt = tx_len;

	return nrfx_uarte_tx(&uarte_inst, tx_buf, tx_len, NRFX_UARTE_TX_BLOCKING);
}
