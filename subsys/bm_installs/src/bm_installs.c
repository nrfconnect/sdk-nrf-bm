...
#if defined(CONFIG_FLASH_HAS_EXPLICIT_ERASE)
	if ((index_offset + sizeof(bm_installs_data)) >= PARTITION_SIZE(metadata_partition)) {
		/* Since this is at end of sector, it would be better to just erase the whole
		 * sector
		 */
		const struct flash_parameters *fparams = flash_get_parameters(metadata_slot.fa_dev);
