...
SETTINGS_STATIC_HANDLER_DEFINE(bluetooth_name, "fw_loader", bluetooth_name_handle_get,
				     ...
				     void *cb_arg)
{
	...
	if (!next) {
		if (!strncmp(name, "adv_name", name_len)) {
			if (len > (sizeof(bluetooth_name_val) - 1)) {
				LOG_ERR("Invalid settings value for fw_loader/adv_name");
				return -EINVAL;
			}
			...
			if (rc < 0) {
				...
				LOG_INF("Config set to %s", bluetooth_name_val);
			}
...
static int bluetooth_name_handle_export(int (*cb)(const char *name, const void *value,
					size_t val_len))
{
	(void)cb("fw_loader/adv_name", bluetooth_name_val, strlen(bluetooth_name_val) + 1);
	LOG_INF("export_done");
	return 0;
}
...
static int bluetooth_name_handle_get(const char *name, char *val, int val_len_max)
{
	...
	if (settings_name_steq(name, "adv_name", &next) && !next) {
