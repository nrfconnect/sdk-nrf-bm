...
/* The maximum number of users/registrations allowed by this module. */
#define DB_DISCOVERY_MAX_USERS CONFIG_BLE_DB_DISCOVERY_MAX_SRV

static ble_uuid_t registered_handlers[DB_DISCOVERY_MAX_USERS];
static ble_db_discovery_evt_handler evt_handler;
/* Pointer to BLE GATT Queue instance. */
static struct ble_gq *gatt_queue = NULL;
				       ...
				       ble_db_discovery_evt_handler evt_handler)
{
	...
	if (num_of_handlers_reg < DB_DISCOVERY_MAX_USERS) {
					   ...
					   uint16_t conn_handle)
{
	...
	if (evt_handler != NULL) {
		if (db_discovery->pending_usr_evt_index < DB_DISCOVERY_MAX_USERS) {
...
uint32_t ble_db_discovery_init(struct ble_db_discovery_init *db_init)
{
	...
	if (!(db_init->evt_handler)) {
		return NRF_ERROR_NULL;
	}

	if (!(db_init->gatt_queue)) {
		return NRF_ERROR_NULL;
	}

	num_of_handlers_reg = 0;
	evt_handler = db_init->evt_handler;
	gatt_queue = db_init->gatt_queue;
