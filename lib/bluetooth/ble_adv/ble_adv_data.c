...
/**
 * @brief AD field in LTV (Length-Type-Value) format.
 *
 * Used by @ref ad_field_encode to write a single AD structure into the advertising data buffer.
 */
struct ad_ltv {
...
/**
 * @brief Common LTV encoder for a single AD field.
 *
 ...
 *
 * @param[in]     ltv      AD field descriptor.
 * @param[out]    buf      Output buffer.
 * @param[in,out] offset   Current write position in buf.
 * @param[in]     max_size Total size of buf.
 *
 * @retval NRF_SUCCESS         AD field written successfully.
				       ...
				       uint16_t max_size)
{
	...
	if (uuid_buf_len > 0) {
		if (uuid_buf_len > (0x00FF - AD_TYPE_FIELD_SIZE)) {
			return NRF_ERROR_DATA_SIZE;
		}
