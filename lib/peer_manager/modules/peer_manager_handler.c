...
static void _conn_secure(uint16_t conn_handle, bool force)
{
	...
	if ((err_code == NRF_SUCCESS) || (err_code == NRF_ERROR_BUSY)) {
		...
		NRF_LOG_WARNING(
			"pm_conn_secure() failed because an SMP timeout is preventing security on "
		...
		NRF_LOG_WARNING(
			"pm_conn_secure() failed because the stored data for conn_handle %d does "
			"not have a valid key.",
			conn_handle);
	} else if (err_code == BLE_ERROR_INVALID_CONN_HANDLE) {
		NRF_LOG_WARNING(
			"pm_conn_secure() failed because conn_handle %d is not a valid connection.",
			conn_handle);
	} else {
...
static void conn_secure(uint16_t conn_handle, bool force)
{
	...
	err_code = app_timer_start(secure_delay_timer, APP_TIMER_TICKS(PM_HANDLER_SEC_DELAY_MS),
				   sec_context.p_void);
	APP_ERROR_CHECK(err_code);
}
...
#define RANK_QUEUE_INIT() PM_PEER_ID_INVALID,
	...
	static pm_peer_id_t rank_queue[8] = {MACRO_REPEAT(RANK_QUEUE_SIZE, RANK_QUEUE_INIT)};
		...
		if (!flash_cleaning) {
			...
			if (!flash_write_after_gc) {
				...
				if (err_code == NRF_ERROR_NOT_FOUND) {
					...
					NRF_LOG_WARNING("Peer ranks functionality is disabled, so "
...
void pm_handler_pm_evt_log(pm_evt_t const *p_pm_evt)
{
	...
	case PM_EVT_BONDED_PEER_CONNECTED:
		NRF_LOG_DEBUG(
			"Previously bonded peer connected: role: %s, conn_handle: %d, peer_id: %d",
	...
	case PM_EVT_CONN_SEC_START:
		NRF_LOG_DEBUG("Connection security procedure started: role: %s, conn_handle: %d, "
			      "procedure: %s",
			      m_roles_str[ble_conn_state_role(p_pm_evt->conn_handle)],
	...
	case PM_EVT_CONN_SEC_FAILED:
		NRF_LOG_INFO("Connection security failed: role: %s, conn_handle: 0x%x, procedure: "
			     "%s, error: %d",
			     m_roles_str[ble_conn_state_role(p_pm_evt->conn_handle)],
			     p_pm_evt->conn_handle,
			     m_sec_procedure_str[p_pm_evt->params.conn_sec_start.procedure],
	...
	case PM_EVT_PEER_DATA_UPDATE_FAILED:
		...
		NRF_LOG_WARNING(
			"Peer data updated failed: peer_id: %d, data_id: %s, action: %s, error: %s",
			p_pm_evt->peer_id,
	...
	case PM_EVT_PEER_DELETE_FAILED:
		NRF_LOG_ERROR("Peer deletion failed: peer_id: %d, error: %s", p_pm_evt->peer_id,
