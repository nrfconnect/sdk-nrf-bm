...
static void peer_data_delete_process(void)
{
	...
	if (peer_id != PM_PEER_ID_INVALID) {
		...
		if (ret == FDS_ERR_NO_SPACE_IN_QUEUES) {
			...
			NRF_LOG_ERROR("Could not delete peer data. fds_file_delete() returned 0x%x "
				      "for peer_id: %d",
				      ret, peer_id);
...
uint32_t pds_init(void)
{
	...
	if (ret != NRF_SUCCESS) {
		NRF_LOG_ERROR("Could not initialize flash storage. fds_register() returned 0x%x.",
			      ret);
			       ...
			       pm_store_token_t *p_store_token)
{
	...
	default:
		NRF_LOG_ERROR(
			"Could not write data to flash. fds_record_{write|update}() returned 0x%x. "
			"peer_id: %d",
			ret, peer_id);
		return NRF_ERROR_INTERNAL;
	}
...
uint32_t pds_peer_data_delete(pm_peer_id_t peer_id, pm_peer_data_id_t data_id)
{
	...
	default:
		NRF_LOG_ERROR(
			"Could not delete peer. fds_record_delete() returned 0x%x. peer_id: %d, "
			"data_id: %d.",
			ret, peer_id, data_id);
		return NRF_ERROR_INTERNAL;
	}
