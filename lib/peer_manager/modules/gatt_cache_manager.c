 ...
 */
static ble_conn_state_user_flag_id_t m_flag_car_update_pending;
/**
 * @brief Flag ID for flag collection to keep track of which connections are pending Central
 *        Address Resolution handle reply.
 */
static ble_conn_state_user_flag_id_t m_flag_car_handle_queried;
/**
 * @brief Flag ID for flag collection to keep track of which connections are pending Central
 *        Address Resolution value reply.
 */
static ble_conn_state_user_flag_id_t m_flag_car_value_queried;

#ifdef PM_SERVICE_CHANGED_ENABLED
STATIC_ASSERT(
	PM_SERVICE_CHANGED_ENABLED || !NRF_SDH_BLE_SERVICE_CHANGED,
...
static void local_db_apply_in_evt(uint16_t conn_handle)
{
	...
	case NRF_ERROR_INVALID_DATA:
		...
		NRF_LOG_WARNING(
			"The local database has changed, so some subscriptions to notifications "
...
static void service_changed_send_in_evt(uint16_t conn_handle)
{
	...
	case NRF_ERROR_INVALID_STATE: {
		...
		if (err_code != NRF_SUCCESS) {
			NRF_LOG_DEBUG("Unexpected error when looking for service changed "
					"CCCD: %s",
...
void store_car_value(uint16_t conn_handle, bool car_value)
{
	...
	if (err_code != NRF_SUCCESS) {
		NRF_LOG_WARNING("CAR char value couldn't be stored (error: %s). Reattempt will "
