...
static pdb_buffer_record_t m_write_buffer_records[PM_FLASH_BUFFERS];
/**
...
static bool write_buf_store_in_event(pdb_buffer_record_t *p_write_buffer_record)
{
	...
	if (err_code != NRF_SUCCESS) {
		...
		if (err_code == NRF_ERROR_STORAGE_FULL) {
			...
			NRF_LOG_ERROR("Some peer data was not properly written to flash. "
				      "write_buf_store() "
			     ...
			     pm_peer_data_t *p_peer_data)
{
	...
	if (p_buffer_memory == NULL) {
		NRF_LOG_ERROR(
			"Cannot store data to flash because pm_buffer_ptr_get() could not retrieve "
