	...
	while (m_current_sc_store_peer_id != PM_PEER_ID_INVALID) {
		...
		if (err_code != NRF_SUCCESS) {
			...
			if (err_code == NRF_ERROR_BUSY) {
				...
				NRF_LOG_ERROR("pds_peer_data_store() returned %s while storing "
					      "service changed "
					      "state for peer id %d.",
		...
		if (err_code == NRF_SUCCESS) {
			...
			if (err_code == NRF_SUCCESS) {
				...
				if ((err_code != NRF_SUCCESS) &&
					...
					NRF_LOG_ERROR("pdb_peer_data_ptr_get() returned %s "
							"for conn_handle: %d",
							nrf_strerror_get(err_code),
							conn_handle);
				...
				if ((err_code == NRF_ERROR_NOT_FOUND) ||
					...
					NRF_LOG_DEBUG("Local db is already up to date, "
							"skipping write.");
...
uint32_t gscm_service_changed_ind_send(uint16_t conn_handle)
{
	...
	if (err_code != NRF_SUCCESS) {
		NRF_LOG_ERROR("sd_ble_gatts_initial_user_handle_get() returned %s which should not "
			      "happen.",
			      nrf_strerror_get(err_code));
		return NRF_ERROR_INTERNAL;
	}
