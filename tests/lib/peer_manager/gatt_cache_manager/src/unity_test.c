...
void read_by_uuid_evt_constuct(ble_evt_t * p_ble_evt, uint16_t conn_handle, uint16_t gatt_status)
{
    ...
    *(uint16_t*)&p_ble_evt->evt.gattc_evt.params.char_val_by_uuid_read_rsp.handle_value[0] = m_arbitrary_handle;
}

void read_evt_constuct(ble_evt_t * p_ble_evt, uint16_t conn_handle, uint16_t gatt_status, uint8_t value)
{
...
static uint32_t ble_conn_state_for_each_set_user_flag_stub(ble_conn_state_user_flag_id_t  flag_id,
...
uint16_t update_flags_check_test(uint16_t n_expected_calls, uint32_t n_call)
{
    ...
    n_calls[n_call] = 8;
    ble_conn_state_for_each_set_user_flag_StubWithCallback(ble_conn_state_for_each_set_user_flag_stub);
    ...
    conn_handle = 6;
    nrf_mtx_trylock_ExpectAndReturn(&m_db_update_in_progress_mutex, true);
    gscm_local_db_cache_update_ExpectAndReturn(conn_handle, NRF_ERROR_INVALID_STATE); // arbitrary unexpected error.
    im_peer_id_get_by_conn_handle_ExpectAndReturn(conn_handle, m_arbitrary_peer_id);
    ble_conn_state_user_flag_set_Expect(conn_handle, m_arbitrary_flag_id_update, false);
    nrf_mtx_unlock_Expect(&m_db_update_in_progress_mutex);
    n_expected_calls++;
...
ret_code_t sd_ble_gatts_value_get_success_stub(uint16_t conn_handle, uint16_t handle, ble_gatts_value_t* p_value, int numCalls, uint16_t cccd_value_in)
{
...
ret_code_t sd_ble_gatts_attr_get_success_stub(uint16_t handle, ble_uuid_t * p_uuid, ble_gatts_attr_md_t * p_md, int numCalls)
{
...
ret_code_t sd_ble_gatts_value_get_success_stub_ind_on(uint16_t conn_handle, uint16_t handle, ble_gatts_value_t* p_value, int numCalls)
{
    return sd_ble_gatts_value_get_success_stub(conn_handle, handle, p_value, numCalls, 2);
}


ret_code_t sd_ble_gatts_value_get_success_stub_ind_off(uint16_t conn_handle, uint16_t handle, ble_gatts_value_t* p_value, int numCalls)
{
...
uint16_t service_changed_flags_check_test(uint16_t n_expected_calls, uint32_t n_call)
{
    ...
    // Success.
    conn_handle = 0;
    ble_conn_state_user_flag_get_ExpectAndReturn(conn_handle, m_arbitrary_flag_id_sc_sent, false);
    ...
    conn_handle = 3;
    ble_conn_state_user_flag_get_ExpectAndReturn(conn_handle, m_arbitrary_flag_id_sc_sent, false);
    gscm_service_changed_ind_send_ExpectAndReturn(conn_handle, BLE_ERROR_GATTS_SYS_ATTR_MISSING);
    ...
    conn_handle = 4;
    ble_conn_state_user_flag_get_ExpectAndReturn(conn_handle, m_arbitrary_flag_id_sc_sent, false);
    gscm_service_changed_ind_send_ExpectAndReturn(conn_handle, NRF_ERROR_FORBIDDEN); // arbitrary unexpected error.
    im_peer_id_get_by_conn_handle_ExpectAndReturn(conn_handle, m_arbitrary_peer_id);
    ble_conn_state_user_flag_set_Expect(conn_handle, m_arbitrary_flag_id_sc, true);
    ble_conn_state_user_flag_set_Expect(conn_handle, m_arbitrary_flag_id_sc_sent, false);
    n_expected_calls++;
...
uint16_t service_changed_flags_check_test_2(uint16_t n_expected_calls, uint32_t n_call)
{
    ...
    // Already sent.
    conn_handle = 1;
    ble_conn_state_user_flag_get_ExpectAndReturn(conn_handle, m_arbitrary_flag_id_sc_sent, true);
...
void test_service_changed_cccd(void)
{
    ...
    ble_uuid_t gatts_uuid_value_wrong = {.uuid = BLE_UUID_GATT_CHARACTERISTIC_SERVICE_CHANGED + 1};
    ...
    TEST_ASSERT_EQUAL_UINT(NRF_ERROR_INVALID_PARAM, service_changed_cccd(m_arbitrary_conn_handle, &cccd_value));
    ...
    sd_ble_gatts_value_get_ExpectAndReturn(m_arbitrary_conn_handle, 2, NULL, NRF_ERROR_INTERNAL);
    sd_ble_gatts_value_get_IgnoreArg_p_value();

    TEST_ASSERT_EQUAL_UINT(NRF_ERROR_INTERNAL, service_changed_cccd(m_arbitrary_conn_handle, &cccd_value));
    ...
    TEST_ASSERT_EQUAL_UINT(NRF_ERROR_NOT_FOUND, service_changed_cccd(m_arbitrary_conn_handle, &cccd_value));
}
...
void test_service_changed_cccd_2(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT(NRF_SUCCESS, service_changed_cccd(m_arbitrary_conn_handle, &cccd_value));
...
void test_car_update_handle(void)
{
    ...
    sd_ble_gattc_char_value_by_uuid_read_ExpectWithArrayAndReturn(m_arbitrary_conn_handle, &car_uuid, 1, &car_handle_range, 1, NRF_SUCCESS);
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_car_hdl, true);

    car_update_pending_handle(m_arbitrary_conn_handle, NULL);

    sd_ble_gattc_char_value_by_uuid_read_ExpectWithArrayAndReturn(m_arbitrary_conn_handle, &car_uuid, 1, &car_handle_range, 1, NRF_ERROR_INTERNAL);
...
uint16_t gcm_local_database_has_changed_test(void)
{
    ...
    {
        peer_id = (peer_id == PM_PEER_ID_INVALID) ? m_arbitrary_peer_id : PM_PEER_ID_INVALID; // Try both valid and invalid peer ids.
        im_peer_id_get_by_conn_handle_ExpectAndReturn(conn_handles.conn_handles[i], peer_id);
        if (peer_id == PM_PEER_ID_INVALID)
        {
            ble_conn_state_user_flag_set_Expect(conn_handles.conn_handles[i], m_arbitrary_flag_id_sc, true);
        }
    }
...
uint16_t apply_flags_check_test(uint16_t n_expected_calls, uint32_t n_call)
{
    ...
    conn_handle = 4;
    gscm_local_db_cache_apply_ExpectAndReturn(conn_handle, NRF_ERROR_INVALID_STATE); // arbitrary unexpected error.
    im_peer_id_get_by_conn_handle_ExpectAndReturn(conn_handle, m_arbitrary_peer_id);
    ble_conn_state_user_flag_set_Expect(conn_handle, m_arbitrary_flag_id_apply, false);
    n_expected_calls++;
...
void test_gcm_ble_evt_handler_BLE_GATTS_EVT_SYS_ATTR_MISSING(void)
{
    ...
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_apply, false);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_apply, apply_pending_handle, NULL, 0);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_sc, sc_send_pending_handle, NULL, 0);
    ...
    gscm_local_db_cache_apply_ExpectAndReturn(m_arbitrary_conn_handle, BLE_ERROR_INVALID_CONN_HANDLE);
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_apply, false);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_apply, apply_pending_handle, NULL, 0);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_sc, sc_send_pending_handle, NULL, 0);
    ...
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_apply, true);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_apply, apply_pending_handle, NULL, 0);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_sc, sc_send_pending_handle, NULL, 0);
    ...
    TEST_ASSERT_EQUAL(m_arbitrary_peer_id, m_evt_handler_records[0].peer_id);
    TEST_ASSERT_EQUAL(m_arbitrary_conn_handle, m_evt_handler_records[0].conn_handle);
    TEST_ASSERT_EQUAL(NRF_ERROR_INVALID_STATE, m_evt_handler_records[0].params.error_unexpected.error);
...
void test_gcm_ble_evt_handler_BLE_GATTS_EVT_SC_CONFIRM(void)
{
    im_peer_id_get_by_conn_handle_ExpectAndReturn(m_arbitrary_conn_handle, m_arbitrary_peer_id);
    gscm_db_change_notification_done_Expect(m_arbitrary_peer_id);
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_sc_sent, false);
...
void test_gcm_ble_evt_handler_BLE_GATTS_EVT_WRITE(void)
{
    ...
    // Success
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_update, true);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_update, db_update_pending_handle, NULL, 0);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_car_upd, car_update_pending_handle, NULL, 0);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_apply, apply_pending_handle, NULL, 0);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_sc, sc_send_pending_handle, NULL, 0);
...
ret_code_t data_store_stub(pm_peer_id_t peer_id, pm_peer_data_const_t const * p_peer_data, pm_store_token_t * p_store_token, int numCalls)
{
...
void test_gcm_ble_evt_handler_BLE_GATTC_EVT_CHAR_VAL_BY_UUID_READ_RSP(void)
{
    ...
    // Not found -> 0
    ble_evt_t * p_ble_evt_err     = read_by_uuid_evt(m_arbitrary_conn_handle, BLE_GATT_STATUS_ATTERR_ATTRIBUTE_NOT_FOUND);
    ble_conn_state_user_flag_get_ExpectAndReturn(m_arbitrary_conn_handle, m_arbitrary_flag_id_car_hdl, true);
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_car_hdl, false);
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_car_upd, false);
    im_peer_id_get_by_conn_handle_ExpectAndReturn(m_arbitrary_conn_handle, m_arbitrary_peer_id);
    ...
    // Success -> not pending
    ble_evt_t * p_ble_evt_success = read_by_uuid_evt(m_arbitrary_conn_handle2, BLE_GATT_STATUS_SUCCESS);
    ble_conn_state_user_flag_get_ExpectAndReturn(m_arbitrary_conn_handle2, m_arbitrary_flag_id_car_hdl, false);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_apply, apply_pending_handle, NULL, 0);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_sc, sc_send_pending_handle, NULL, 0);
    ...
    ble_conn_state_user_flag_get_ExpectAndReturn(m_arbitrary_conn_handle, m_arbitrary_flag_id_car_hdl, true);
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_car_hdl, false);
    sd_ble_gattc_read_ExpectAndReturn(m_arbitrary_conn_handle, m_arbitrary_handle, 0, NRF_ERROR_INTERNAL);
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_car_upd, false);
    im_peer_id_get_by_conn_handle_ExpectAndReturn(m_arbitrary_conn_handle, m_arbitrary_peer_id);
    pds_peer_data_store_StubWithCallback(data_store_stub);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_apply, apply_pending_handle, NULL, 0);
    ...
    ble_conn_state_user_flag_get_ExpectAndReturn(m_arbitrary_conn_handle2, m_arbitrary_flag_id_car_hdl, true);
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle2, m_arbitrary_flag_id_car_hdl, false);
    sd_ble_gattc_read_ExpectAndReturn(m_arbitrary_conn_handle2, m_arbitrary_handle, 0, NRF_SUCCESS);
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle2, m_arbitrary_flag_id_car_val, true);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_apply, apply_pending_handle, NULL, 0);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_sc, sc_send_pending_handle, NULL, 0);
...
{
    // Success -> not pending
    ble_evt_t * p_ble_evt_success = read_evt(m_arbitrary_conn_handle2, BLE_GATT_STATUS_SUCCESS, 0);
    ble_conn_state_user_flag_get_ExpectAndReturn(m_arbitrary_conn_handle2, m_arbitrary_flag_id_car_val, false);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_apply, apply_pending_handle, NULL, 0);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_sc, sc_send_pending_handle, NULL, 0);
    ...
    ble_conn_state_user_flag_get_ExpectAndReturn(m_arbitrary_conn_handle, m_arbitrary_flag_id_car_val, true);
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_car_val, false);
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_car_upd, false);
    im_peer_id_get_by_conn_handle_ExpectAndReturn(m_arbitrary_conn_handle, m_arbitrary_peer_id);
    pds_peer_data_store_StubWithCallback(data_store_stub);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_apply, apply_pending_handle, NULL, 0);
...
void test_im_evt_handler(void)
{
    ...
    im_peer_id_get_by_conn_handle_ExpectAndReturn(m_arbitrary_conn_handle, m_arbitrary_peer_id);
    pds_peer_data_read_ExpectAndReturn(m_arbitrary_peer_id, PM_PEER_DATA_ID_CENTRAL_ADDR_RES, NULL, NULL, NRF_SUCCESS);
...
void test_pdb_evt_handler(void)
{
    ...
    *(bool*)returned_peer_data.p_service_changed_pending = false;
    ...
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_update, true);
    im_peer_id_get_by_conn_handle_ExpectAndReturn(m_arbitrary_conn_handle, m_arbitrary_peer_id);
    pds_peer_data_read_ExpectAndReturn(m_arbitrary_peer_id, PM_PEER_DATA_ID_CENTRAL_ADDR_RES, NULL, NULL, NRF_ERROR_NOT_FOUND);
    pds_peer_data_read_IgnoreArg_p_data();
    ble_conn_state_user_flag_set_Expect(m_arbitrary_conn_handle, m_arbitrary_flag_id_car_upd, true);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_update, db_update_pending_handle, NULL, 0);
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_car_upd, car_update_pending_handle, NULL, 0);
    ...
    // OTHER
    ble_conn_state_for_each_set_user_flag_ExpectAndReturn(m_arbitrary_flag_id_update, db_update_pending_handle, NULL, 1);
