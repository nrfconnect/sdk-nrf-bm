...
void test_ble_scan_filter_set_get(void)
{
	uint32_t nrf_err;
	...
	ble_scan_filter_set(&ble_scan, BLE_SCAN_NAME_FILTER, device_name);
...
void test_ble_scan_filter_set_error_null(void)
{
	uint32_t nrf_err;
	char *device_name = "generic_device";
	...
	nrf_err = ble_scan_filter_set(NULL, (BLE_SCAN_NAME_FILTER), device_name);
	TEST_ASSERT_EQUAL(NRF_ERROR_NULL, nrf_err);

	nrf_err = ble_scan_filter_set(&ble_scan, (BLE_SCAN_NAME_FILTER), NULL);
	TEST_ASSERT_EQUAL(NRF_ERROR_NULL, nrf_err);
}

void test_ble_scan_filter_set_error_invalid_param(void)
{
	uint32_t nrf_err;
	...
	nrf_err = ble_scan_filter_set(&ble_scan, 0, device_name);
	TEST_ASSERT_EQUAL(NRF_ERROR_INVALID_PARAM, nrf_err);
}

void test_ble_scan_filter_set_name(void)
{
	uint32_t nrf_err;
	char *device_name = DEVICE_NAME;
...
void test_ble_scan_filter_set_addr(void)
{
	uint32_t nrf_err;
...
void test_ble_scan_filter_set_addr_enomem(void)
{
	uint32_t nrf_err;
	uint8_t addr[BLE_GAP_ADDR_LEN] = {0xa, 0xd, 0xd, 0x4, 0xe, 0x5};

	test_ble_scan_filter_set_addr();

	nrf_err = ble_scan_filter_set(&ble_scan, BLE_SCAN_ADDR_FILTER, addr);
	TEST_ASSERT_EQUAL(NRF_ERROR_NO_MEM, nrf_err);
}

void test_ble_scan_filter_set_uuid(void)
{
	uint32_t nrf_err;
	ble_uuid_t uuid = {
		.uuid = UUID,
	...
	nrf_err = ble_scan_filter_set(&ble_scan, BLE_SCAN_UUID_FILTER, &uuid);
	TEST_ASSERT_EQUAL(NRF_SUCCESS, nrf_err);
}

void test_ble_scan_filter_set_uuid_error_no_mem(void)
{
	uint32_t nrf_err;
	ble_uuid_t uuid;

	test_ble_scan_filter_set_uuid();

	nrf_err = ble_scan_filter_set(&ble_scan, BLE_SCAN_UUID_FILTER, &uuid);
	TEST_ASSERT_EQUAL(NRF_ERROR_NO_MEM, nrf_err);
}

void test_ble_scan_filter_set_appearance(void)
{
	uint32_t nrf_err;
	...
	nrf_err = ble_scan_filter_set(&ble_scan, BLE_SCAN_APPEARANCE_FILTER, &appearance);
	TEST_ASSERT_EQUAL(NRF_SUCCESS, nrf_err);
}

void test_ble_scan_filter_set_appearance_error_no_mem(void)
{
	uint32_t nrf_err;
	uint16_t appearance = 0xa44e;

	test_ble_scan_filter_set_appearance();

	nrf_err = ble_scan_filter_set(&ble_scan, BLE_SCAN_APPEARANCE_FILTER, &appearance);
	TEST_ASSERT_EQUAL(NRF_ERROR_NO_MEM, nrf_err);
}

void test_ble_scan_filter_set_short_name(void)
{
	uint32_t nrf_err;
	...
	nrf_err = ble_scan_filter_set(&ble_scan, BLE_SCAN_SHORT_NAME_FILTER, &short_name);
	TEST_ASSERT_EQUAL(NRF_SUCCESS, nrf_err);
}

void test_ble_scan_filter_set_short_name_error_no_mem(void)
{
	uint32_t nrf_err;
	struct ble_scan_short_name short_name = {
		.short_name = "dev",
	...
	test_ble_scan_filter_set_short_name();
	...
	nrf_err = ble_scan_filter_set(&ble_scan, BLE_SCAN_SHORT_NAME_FILTER, &short_name2);
	TEST_ASSERT_EQUAL(NRF_SUCCESS, nrf_err);
...
void test_is_whitelist_used(void)
{
	...
	nrf_err = is_whitelist_used(&ble_scan);
...
void test_ble_scan_on_ble_evt_adv_report_device_uuid_connect(void)
{
	...
	__cmock_sd_ble_gap_connect_ExpectWithArrayAndReturn(
		...
		&scan_cfg_with_params.scan_param, 1,
		&scan_cfg_with_params.conn_param, 1,
