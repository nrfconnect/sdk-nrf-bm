...
#define VALID_CONN_HANDLE 0x0C4
...
void test_ble_gq_item_add_req_gatts_hvx_invalid_addr(void)
{
	uint8_t data[] = { 0x01, 0x02, 0x03 };
	uint16_t len = sizeof(data);
	uint16_t conn_handle = 0;
...
void test_ble_gq_item_add_req_gatts_hvx_invalid_param(void)
{
	uint8_t data[] = { 0x01, 0x02, 0x03 };
	uint16_t len = sizeof(data);
	uint16_t conn_handle = 0;
...
void test_ble_gq_conn_handle_register_enomem(void)
{
	...
	TEST_ASSERT_EQUAL(0, ret);
	ret = ble_gq_conn_handle_register(&ble_gq, 1);
	TEST_ASSERT_EQUAL(0, ret);
	ret = ble_gq_conn_handle_register(&ble_gq, 2);
	TEST_ASSERT_EQUAL(NRF_ERROR_NO_MEM, ret);
}
...
void test_ble_gq_on_ble_evt_gap_evt_disconnected(void)
{
	ble_evt_t ble_evt = {0};

	ble_evt.header.evt_id = BLE_GAP_EVT_DISCONNECTED;
	ble_evt.evt.gap_evt.conn_handle = VALID_CONN_HANDLE;
	ble_gq.conn_handles[0] = VALID_CONN_HANDLE;
	...
	for (int i = 0; i < ble_gq.max_conns; i++) {
		TEST_ASSERT_NOT_EQUAL(ble_gq.purge_list[i], VALID_CONN_HANDLE);
	}
}

void test_ble_gq_on_ble_evt_gatc_evt_read_rsp(void)
{
	ble_evt_t ble_evt = {0};

	ble_evt.header.evt_id = BLE_GATTC_EVT_READ_RSP;
	ble_evt.evt.gattc_evt.conn_handle = VALID_CONN_HANDLE;
	ble_gq.conn_handles[0] = VALID_CONN_HANDLE;
	...
	__cmock_sd_ble_gattc_read_IgnoreAndReturn(NRF_ERROR_BUSY);
	ble_gq_item_add(&ble_gq, &req, VALID_CONN_HANDLE);
	__cmock_sd_ble_gattc_read_ExpectAndReturn(VALID_CONN_HANDLE, req.gattc_read.handle,
			req.gattc_read.offset, NRF_ERROR_TIMEOUT);
	ble_gq_on_ble_evt(&ble_evt, (void *)&ble_gq);
	TEST_ASSERT_EQUAL(VALID_CONN_HANDLE, glob_conn_handle);
	TEST_ASSERT_EQUAL(NRF_ERROR_TIMEOUT, glob_error);
}
