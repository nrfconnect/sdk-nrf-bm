...
static char_write_t char_write_evt;
...
static void emulate_ble_write_evt(struct ble_hids *ble_hids, on_write_evt_t on_write_gatts,
...
static void emulate_ble_rw_authorize_evt(struct ble_hids *ble_hids,
					 on_rw_auth_evt_t auth_evt_type)
{
	ble_evt_t ble_evt = {
...
void test_struct_ble_hids_configoo_much_rep(void)
{
	uint32_t err;
	struct ble_hids_config ble_hids_init_obj = {};
	struct ble_hids ble_hids = {
...
void test_ble_hids_init_kb_correct(void)
{
	...
	TEST_ASSERT_NULL(ble_hids.feature_rep_init_array);
...
void test_on_control_point(void)
{
	struct ble_hids ble_hids = INITIALIZED_HIDS;
	void *host;
	uint8_t data[] = {HIDS_CONTROL_POINT_SUSPEND};
	uint16_t len = sizeof(data);
	struct ble_hids_client_context *p_contex_data;
	...
	p_contex_data = (struct ble_hids_client_context *)host;

	p_contex_data->ctrl_pt = 0x98;
	...
	TEST_ASSERT_EQUAL(HIDS_CONTROL_POINT_SUSPEND, p_contex_data->ctrl_pt);
