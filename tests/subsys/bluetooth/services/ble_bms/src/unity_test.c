...
void test_ble_bms_set_conn_handle_error_null(void)
{
	uint32_t nrf_err;

	nrf_err = ble_bms_set_conn_handle(NULL, CONN_HANDLE);
	TEST_ASSERT_EQUAL(NRF_ERROR_NULL, nrf_err);
}

void test_ble_bms_set_conn_handle(void)
{
	uint32_t nrf_err;

	nrf_err = ble_bms_set_conn_handle(&ble_bms, CONN_HANDLE);
	TEST_ASSERT_EQUAL(NRF_SUCCESS, nrf_err);
}
