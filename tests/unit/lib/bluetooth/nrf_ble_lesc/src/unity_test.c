...
void test_nrf_ble_lesc_init_error_internal(void)
{
	...
	nrf_err = nrf_ble_lesc_init();
	TEST_ASSERT_EQUAL_UINT32(NRF_ERROR_INTERNAL, nrf_err);
}
...
static uint32_t callback_sd_ble_gap_lesc_dhkey_reply_success(uint16_t conn_handle,
	uint8_t sec_status, ble_gap_lesc_dhkey_t const *p_dhkey, int cmock_num_calls)
{
	ARG_UNUSED(cmock_num_calls);
...
void test_nrf_ble_lesc_compute_and_give_dhkey_success(void)
{
	...
	__cmock_sd_ble_gap_lesc_dhkey_reply_ExpectAndReturn(
		conn_handle, BLE_GAP_SEC_STATUS_SUCCESS, PTR_IGNORE, VAL_IGNORE);
...
void test_nrf_ble_lesc_compute_and_give_dhkey_with_invalid_peer_key(void)
{
	...
	__cmock_sd_ble_gap_lesc_dhkey_reply_ExpectAndReturn(
		conn_handle, BLE_GAP_SEC_STATUS_DHKEY_FAILURE, PTR_IGNORE, VAL_IGNORE);
	__cmock_sd_ble_gap_lesc_dhkey_reply_IgnoreArg_p_dhkey();
	__cmock_sd_ble_gap_lesc_dhkey_reply_AddCallback(
		callback_sd_ble_gap_lesc_dhkey_reply_success);
...
void tearDown(void)
{
	...
	/* Clear the internal exported public key before the next test. */
	...
	/* Stubs have priority. Clear stubs so that the following expect functions are used. */
	...
	/* Reset generated key state before the next test. */
	__cmock_psa_crypto_init_ExpectAndReturn(PSA_SUCCESS);
