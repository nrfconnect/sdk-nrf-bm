...
void test_bm_storage_uninit_ebusy(void)
{
	int err;
	...
	/* Instance has a pending operation */
	err = bm_storage_uninit(&storage);
...
{
	int err;
	uint8_t buf[BLOCK_SIZE] = "Ciao";
	uint8_t buf_large[BLOCK_SIZE * 4] = "Ciao";
	struct bm_storage storage = {0};
	struct bm_storage_config config = {
		.evt_handler = bm_storage_evt_handler,
		.start_addr = PARTITION_START,
	...
	err = bm_storage_write(&storage, PARTITION_START - 1, buf, sizeof(buf), NULL);
	TEST_ASSERT_EQUAL(-EFAULT, err);

	/* Operation is out of bounds. */
	err = bm_storage_write(&storage, PARTITION_START + 1, buf_large, sizeof(buf_large), NULL);
	TEST_ASSERT_EQUAL(-EFAULT, err);
}
...
void test_bm_storage_write_queue_full(void)
{
	int err;
	uint8_t buf[BLOCK_SIZE];
	struct bm_storage storage = {0};
...
void test_bm_storage_write_queue_uninit(void)
{
	int err;
	uint8_t buf[BLOCK_SIZE];
	struct bm_storage storage = {0};
	...
	/* Can't uninit with something in the queue */
	err = bm_storage_uninit(&storage);
	TEST_ASSERT_EQUAL(-EBUSY, err);
...
void test_bm_storage_write_is_busy(void)
{
	int err;
	bool is_busy;
	uint8_t buf[BLOCK_SIZE];
...
void test_bm_storage_read_efault(void)
{
	int err;
	uint8_t buf[BLOCK_SIZE];
	uint8_t buf_large[BLOCK_SIZE * 4];
	struct bm_storage storage = {0};
	struct bm_storage_config config = {
		.evt_handler = bm_storage_evt_handler,
		.start_addr = PARTITION_START,
	...
	err = bm_storage_read(NULL, PARTITION_START, buf, sizeof(buf));
	TEST_ASSERT_EQUAL(-EFAULT, err);

	err = bm_storage_read(&storage, PARTITION_START, NULL, sizeof(buf));
	TEST_ASSERT_EQUAL(-EFAULT, err);
	...
	/* Operation is out of bounds. */
	err = bm_storage_read(&storage, PARTITION_START, buf_large, sizeof(buf_large));
	TEST_ASSERT_EQUAL(-EFAULT, err);
}
...
void test_bm_storage_erase(void)
{
	...
	for (size_t i = 0; i < WORD_SIZE(BLOCK_SIZE); i++) {
		__cmock_sd_flash_write_ExpectAndReturn(
			(uint32_t *)(PARTITION_START + (i * bm_storage_info.erase_unit)),
			&bm_storage_info.erase_value, WORD_SIZE(BLOCK_SIZE), 0);
	}
