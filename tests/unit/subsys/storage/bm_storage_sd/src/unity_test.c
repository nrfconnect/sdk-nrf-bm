...
void test_bm_storage_init_deprecated(void)
{
	int err;
	struct bm_storage storage = {0};
	struct bm_storage_config config = {
...
void test_bm_storage_write_einval(void)
{
	...
	/* Unaligned length */
	err = bm_storage_write(&storage, ADDR, buf, sizeof(buf) - 1, NULL);
	TEST_ASSERT_EQUAL(-EINVAL, err);

	/* Unaligned source */
	err = bm_storage_write(&storage, ADDR, buf + 1, sizeof(buf), NULL);
	TEST_ASSERT_EQUAL(-EINVAL, err);
}
...
void test_bm_storage_write_queued(void)
{
	...
	err = bm_storage_write(&storage, ADDR, buf2, sizeof(buf2), NULL);
	TEST_ASSERT_EQUAL(0, err);
...
void test_bm_storage_write_retry_queued(void)
{
	...
	err = bm_storage_write(&storage, ADDR, buf, sizeof(buf), (void *)0xDEADBEEF);
	TEST_ASSERT_EQUAL(0, err);

	err = bm_storage_write(&storage, ADDR, buf, sizeof(buf), (void *)0x0FA7FACE);
	TEST_ASSERT_EQUAL(0, err);
 ...
 */
void test_bm_storage_write_queued_eio(void)
{
	int err;
	uint8_t buf[BLOCK_SIZE];
	struct bm_storage storage = {0};
	...
	err = bm_storage_write(&storage, ADDR, buf, sizeof(buf), (void *)0xBEEFDEAD);
	TEST_ASSERT_EQUAL(0, err);
	...
	TEST_ASSERT_EQUAL(ADDR, storage_events[0].addr);
	...
	TEST_ASSERT_EQUAL(ADDR, storage_events[1].addr);
...
void test_bm_storage_write_queued_disable_prepare_busy(void)
{
	...
	TEST_ASSERT_EQUAL(BM_STORAGE_EVT_WRITE_RESULT, storage_event.id);
	TEST_ASSERT_EQUAL(BM_STORAGE_EVT_DISPATCH_MODE_SYNC, storage_event.dispatch_mode);
	TEST_ASSERT_EQUAL(0, storage_event.result);
	TEST_ASSERT_EQUAL(ADDR, storage_event.addr);
	TEST_ASSERT_EQUAL_PTR(buf2, storage_event.src);
	TEST_ASSERT_EQUAL(sizeof(buf2), storage_event.len);
...
void test_bm_storage_write_two_instances(void)
{
	...
	err = bm_storage_write(&storage, ADDR, buf, sizeof(buf), &storage);
	TEST_ASSERT_EQUAL(0, err);
	...
	err = bm_storage_write(&storage2, ADDR, buf, sizeof(buf), &storage2);
	TEST_ASSERT_EQUAL(0, err);
...
void test_bm_storage_erase_einval(void)
{
	...
	err = bm_storage_erase(&storage, ADDR, BLOCK_SIZE + 1, NULL);
	TEST_ASSERT_EQUAL(-EINVAL, err);
}
...
void test_bm_storage_erase_chunk(void)
{
	...
	err = bm_storage_erase(&storage, ADDR, big_block, NULL);
	TEST_ASSERT_EQUAL(0, err);
...
void test_bm_storage_erase_odd(void)
{
	...
	err = bm_storage_erase(&storage, ADDR, small_block, NULL);
	TEST_ASSERT_EQUAL(0, err);
...
void test_bm_storage_erase_enomem(void)
{
	...
	for (size_t i = 0; i < CONFIG_BM_STORAGE_BACKEND_SD_QUEUE_SIZE + 1; i++) {
		err = bm_storage_erase(&storage, ADDR, storage.nvm_info->erase_unit, NULL);
		TEST_ASSERT_EQUAL(0, err);
	}
