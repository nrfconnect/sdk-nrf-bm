...
void test_ble_nus_data_send_notif_disabled(void)
{
	uint32_t nrf_err;
	uint8_t data[2] = {0x01, 0x02};
	uint16_t length = sizeof(data);
...
void test_ble_nus_data_send_sys_attr_missing(void)
{
	uint32_t nrf_err;
	uint8_t data[2] = {0x01, 0x02};
	uint16_t length = sizeof(data);
	...
	nrf_err = ble_nus_data_send(&ble_nus, data, &length, test_case_conn_handle);
	TEST_ASSERT_EQUAL(BLE_ERROR_GATTS_SYS_ATTR_MISSING, nrf_err);
}
