...
void test_ble_nus_client_tx_notif_enable(void)
{
	...
	struct ble_gatt_db_srv discovered_service = {
		.srv_uuid = {.uuid = BLE_UUID_NUS_SERVICE, .type = ble_nus_client.uuid_type},
		.char_count = 2,
		.characteristics = {{.characteristic.uuid.uuid = BLE_UUID_NUS_RX_CHARACTERISTIC,
				     .cccd_handle = 0x100},
				    {.characteristic.uuid.uuid = BLE_UUID_NUS_TX_CHARACTERISTIC,
				     .cccd_handle = 0x100}}};
	struct ble_db_discovery_evt db_evt = {.conn_handle = test_case_conn_handle,
	...
	ble_nus_client_handles_assign(&ble_nus_client, db_evt.conn_handle,
				      &nus_client_evt.discovery_complete);
...
void test_ble_nus_client_handles_assign(void)
{
	...
	struct ble_gatt_db_srv discovered_service = {
		...
		.characteristics = {{.characteristic.uuid.uuid = BLE_UUID_NUS_RX_CHARACTERISTIC},
				    {.characteristic.uuid.uuid = BLE_UUID_NUS_TX_CHARACTERISTIC}}};
	struct ble_db_discovery_evt db_evt = {.conn_handle = 100,
					      .evt_type = BLE_DB_DISCOVERY_COMPLETE,
					      .discovered_db = &discovered_service};
...
void test_ble_nus_client_on_ble_evt(void)
{
	...
	TEST_ASSERT_EQUAL(BLE_NUS_CLIENT_EVT_TX_EVT, nus_client_evt.evt_type);
	TEST_ASSERT_EQUAL(ble_evt.evt.gattc_evt.params.hvx.data[0],
			  nus_client_evt.tx_data.data[0]);
	TEST_ASSERT_EQUAL(ble_evt.evt.gattc_evt.params.hvx.len, nus_client_evt.tx_data.data_len);
