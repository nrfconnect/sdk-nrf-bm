	...
	for (int i = 0; i < data_len; i++) {
		...
		if ((c == '\n' || c == '\r') || (rx_buf_idx >= sizeof(rx_buf))) {
			...
			if (nrfx_err != NRFX_SUCCESS) {
				LOG_ERR("nrfx_uarte_tx failed, nrfx_err %d", nrfx_err);
			}
...
static nrfx_err_t uarte_init(void)
{
	...
	if (nrfx_err != NRFX_SUCCESS) {
		LOG_ERR("Failed to initialize UARTE, nrfx_err %d", nrfx_err);
		return nrfx_err;
	}
...
int main(void)
{
	...
	nrfx_err = uarte_init();
	if (nrfx_err != NRFX_SUCCESS) {
		LOG_ERR("Failed to enable UARTE, nrfx_err %d", nrfx_err);
		goto idle;
	}
	...
	if (nrfx_err != NRFX_SUCCESS) {
		LOG_ERR("UARTE TX failed, nrfx_err %d", nrfx_err);
		goto idle;
	}
	...
	if (nrfx_err != NRFX_SUCCESS) {
		LOG_ERR("UARTE RX failed, nrfx_err %d", nrfx_err);
	}
