...
{
	shell_print(sh, "The modulated TX has finished");
}
...
static void rx_end(void)
{
	...
	if (req_pkt == 0 || req_pkt < recv_pkt) {
		shell_print(sh, "Error receiving packets");
		return;
	}

	shell_print(sh, "");
	shell_print(sh, "Received number of packets: %d", recv_pkt);
	shell_print(sh, "Required number of packets: %d", req_pkt);
}
...
struct radio_test_config *radio_cmd_get_test_config(void)
{
	return &test_config;
}
