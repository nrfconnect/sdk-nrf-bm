...
enum rx_timeout_state {
	RX_TIMEOUT_INACTIVE,
	RX_TIMEOUT_IMMEDIATE,
	RX_TIMEOUT_DELAYED,
};
static volatile enum rx_timeout_state rx_timeout_state;
...
static nrf_radio_txpower_t dbm_to_nrf_radio_txpower(int8_t tx_power)
{
	...
	default:
		shell_print(sh, "TX power to enumerator conversion failed, defaulting to 0 dBm");
		return RADIO_TXPOWER_TXPOWER_0dBm;
	}
}
...
static void radio_power_set(nrf_radio_mode_t mode, uint8_t channel, int8_t power)
{
	...
	if (!sweep_processing) {
		shell_print(sh, "Requested tx output power: %" PRIi8 " dBm", power);
		shell_print(sh, "Tx output power set to: %" PRIi8 " dBm", output_power);
	}
}
		     ...
		     uint32_t rx_packet_num)
{
	...
	if (rx_packet_num > 0) {
		rx_timeout_schedule(CONFIG_SAMPLE_RADIO_TEST_RX_TIMEOUT * 1000);
	}
}
...
static void timer_handler(nrf_timer_event_t event_type, void *context)
{
	...
	if (event_type == NRF_TIMER_EVENT_COMPARE0) { /* sweep test running */
		...
		if (config->type == TX_SWEEP) {
			...
			shell_print(sh, "Unexpected test type: %d", config->type);
			return;
		}
...
static void timer_init(const struct radio_test_config *config)
{
	...
	if (err != 0) {
		shell_print(sh, "nrfx_timer_init failed with: %d", err);
	}
}
...
int radio_test_init(struct radio_test_config *config)
{
	...
	if (nrfx_err != 0) {
		shell_print(sh, "Failed to allocate gppi channel.");
		return -EFAULT;
	}
...
void radio_test_process_rx_timeout(void)
{
