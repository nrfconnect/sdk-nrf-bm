...
LOG_MODULE_REGISTER(sample, CONFIG_SAMPLE_PPI_LOG_LEVEL);
...
int main(void)
{
	...
	uint32_t ticks_blink_period;

	nrfx_gpiote_t *gpiote_inst =
		bm_gpiote_instance_get(NRF_PIN_NUMBER_TO_PORT(BOARD_PIN_LED_1));

	nrfx_timer_config_t timer_config = {
		/* For frequencies up to 1MHz, the timer peripheral uses the 1MHz clock instead of
		 * the 16MHz clock.
		 */
		.frequency = 1000000,
	...
	ticks_blink_period = nrfx_timer_ms_to_ticks(&timer_inst,
						    CONFIG_SAMPLE_LED_BLINK_INTERVAL_MS);

	nrfx_timer_extended_compare(&timer_inst, NRF_TIMER_CC_CHANNEL0, ticks_blink_period,
	...
	/* We don't want to wake up the CPU on timer compare event. */
