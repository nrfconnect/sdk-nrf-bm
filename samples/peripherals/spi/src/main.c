...
int main(void)
{
	...
	if (err) {
		LOG_ERR("SPIM init failed, err %d", err);
		goto idle;
	}
	...
	if (err) {
		LOG_ERR("SPIS init failed, err %d", err);
		goto idle;
	}

	err = nrfx_spis_buffers_set(&spis_inst, NULL, 0, rx_buf, sizeof(rx_buf));
	if (err) {
		LOG_ERR("SPIS buffers set failed, err %d", err);
		goto idle;
	}

	err = bm_buttons_enable();
	if (err) {
		LOG_ERR("Buttons enable failed, err %d", err);
		goto idle;
	}
