...
Its SPI pins and strap pins, which configure fixed hardware settings such as write-protect and reset state at power on, are defined by the ``BOARD_EXTERNAL_MEMORY_*`` macros in :file:`board-config.h`.
