...
static void lpuarte_event_handler(nrfx_uarte_event_t const *event, void *ctx)
{
	...
	case NRFX_UARTE_EVT_RX_BUF_REQUEST:
		nrf_err = bm_lpuarte_rx_buffer_set(lpu, &uarte_rx_buf[buf_idx * 128], 128);
		buf_idx++;
...
static uint32_t lpuarte_init(void)
{
	uint32_t nrfx_err;
...
int main(void)
{
	...
	if (nrfx_err != NRFX_SUCCESS) {
		LOG_ERR("Failed to enable UARTE, nrf_error %#x", nrfx_err);
		goto idle;
	}
	...
	if (nrfx_err != NRFX_SUCCESS) {
		LOG_ERR("UARTE RX failed, nrfx_error %#x", nrfx_err);
		goto idle;
	}
