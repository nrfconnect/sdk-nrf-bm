...
To test this sample, you need the following:

* A computer with `nRF Connect for Desktop`_ with the `Bluetooth Low Energy app`_ and the `Serial Terminal app` installed.
* A mobile device with the `nRF Toolbox`_ app.

1. Connect the device to the computer and open the Serial Terminal.
#. Connect to the kit with a terminal emulator to the UART.
...
#. In the Serial Terminal, observe that the ``Continuous Glucose Monitoring sample started.`` message is printed.
   Note that the kit has two COM ports (Windows) or ttyACM devices (Linux), where only one will output the log.
