...
/* TODO: Documentation, bas client, bas server, body sensor location */
...
static uint16_t central_conn_handle = BLE_CONN_HANDLE_INVALID;
static uint16_t peripheral_conn_handle = BLE_CONN_HANDLE_INVALID;
...
static void on_ble_evt(const ble_evt_t *ble_evt, void *ctx)
{
	...
	case BLE_GAP_EVT_CONNECTED:
		...
		if (gap_evt->params.connected.role == BLE_GAP_ROLE_CENTRAL) {
			peripheral_conn_handle = ble_evt->evt.gap_evt.conn_handle;
	...
	case BLE_GAP_EVT_DISCONNECTED:
		...
		if (central_conn_handle == gap_evt->conn_handle) {
			central_conn_handle = BLE_CONN_HANDLE_INVALID;
			nrf_gpio_pin_write(BOARD_PIN_LED_2, !BOARD_LED_ACTIVE_STATE);
		}
		if (peripheral_conn_handle == gap_evt->conn_handle) {
			peripheral_conn_handle = BLE_CONN_HANDLE_INVALID;
...
static uint32_t adv_init(void)
{
	...
	struct ble_adv_config ble_adv_cfg = {
		...
		.adv_data = {.name_type = BLE_ADV_DATA_FULL_NAME,
...
static void button_handler_disconnect_peripheral(uint8_t pin, uint8_t action)
{
	if (peripheral_conn_handle == BLE_CONN_HANDLE_INVALID || action != BM_BUTTONS_PRESS) {
		return;
	}

	LOG_INF("Button disconnect peripheral");

	uint32_t nrf_err = sd_ble_gap_disconnect(peripheral_conn_handle,
						 BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION);
...
static void button_handler_disconnect_central(uint8_t pin, uint8_t action)
{
	if (central_conn_handle == BLE_CONN_HANDLE_INVALID || action != BM_BUTTONS_PRESS) {
		return;
	}

	LOG_INF("Button disconnect central");

	uint32_t nrf_err = sd_ble_gap_disconnect(central_conn_handle,
						 BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION);
...
static void hrs_c_evt_handler(struct ble_hrs_client *hrs, const struct ble_hrs_client_evt *evt)
{
	...
	case BLE_HRS_CLIENT_EVT_DISCOVERY_COMPLETE:
		ble_hrs_body_sensor_location_set(&ble_hrs, evt->discovery_complete.body_sensor_location);

		nrf_err = ble_hrs_client_handles_assign(hrs, evt->conn_handle, &evt->discovery_complete.handles);
		if (nrf_err != 0) {
	...
	case BLE_HRS_CLIENT_EVT_HRM_NOTIFICATION:
		...
		nrf_err = ble_hrs_heart_rate_measurement_send(&ble_hrs, evt->hrm_notification.hr_value);
		if (nrf_err) {
	...
	case BLE_HRS_CLIENT_EVT_BSL_UPDATE:
		ble_hrs_body_sensor_location_set(&ble_hrs, evt->body_sensor_location);
		break;
...
static void ble_hrs_evt_handler(struct ble_hrs *hrs, const struct ble_hrs_evt *evt)
{
	...
	case BLE_HRS_EVT_ERROR:
		LOG_ERR("HRS error event, nrf_err %#x", evt->error.reason);
		break;
	default:
		break;
	}
...
static void ble_bas_c_evt_handler(struct ble_bas_client *bas, const struct ble_bas_client_evt *evt)
{
	uint32_t nrf_err;
	...
	case BLE_BAS_CLIENT_EVT_DISCOVERY_COMPLETE:
		...
		nrf_err = ble_bas_client_handles_assign(bas, evt->conn_handle, &evt->discovery_complete.handles);
		if (nrf_err != 0) {
	...
	case BLE_BAS_CLIENT_EVT_BATT_NOTIFICATION:
		...
		nrf_err = ble_bas_battery_level_update(&ble_bas, central_conn_handle, evt->battery.level);
		if (nrf_err) {
		/* Ignore if not connected, or CCCD not written/configured by peer. */
			if (nrf_err != BLE_ERROR_INVALID_CONN_HANDLE &&
		    	nrf_err != NRF_ERROR_INVALID_STATE &&
...
static void ble_bas_evt_handler(struct ble_bas *bas, const struct ble_bas_evt *evt)
{
	...
	case BLE_BAS_EVT_ERROR:
		LOG_ERR("BAS error event %d", evt->error.reason);
		break;
	default:
		break;
	}
...
{
	uint32_t nrf_err;
	int8_t body_sensor_location = BLE_HRS_BODY_SENSOR_LOCATION_CHEST;
	...
	if (nrf_err) {
		LOG_ERR("Failed to init HRS server, nrf_error %#x", nrf_err);
	}
...
{
	uint32_t nrf_err;
	struct ble_bas_client_config bas_c_cfg = {
		.db_discovery = &ble_db_disc,
		.evt_handler = ble_bas_c_evt_handler,
		.gatt_queue = &ble_gq
	};

	nrf_err = ble_bas_client_init(&ble_bas_client, &bas_c_cfg);
	return nrf_err;
}
...
int main(void)
{
	...
	static struct bm_buttons_config configs[] = {
		...
		},
		{
			...
			.active_state = BM_BUTTONS_ACTIVE_LOW,
			.pull_config = BM_BUTTONS_PIN_PULLUP,
			.handler = button_handler_disconnect_peripheral,
		},
		{
			.pin_number = BOARD_PIN_BTN_2,
			.active_state = BM_BUTTONS_ACTIVE_LOW,
			.pull_config = BM_BUTTONS_PIN_PULLUP,
			.handler = button_handler_disconnect_central,
		},
	};
