...
/* TODO: Documentation, fix resetting this device. name filter, bas client, bas server */
...
static void hrs_c_evt_handler(struct ble_hrs_client *hrs, struct ble_hrs_client_evt *evt)
{
	...
	case BLE_HRS_CLIENT_EVT_DISCOVERY_COMPLETE:
		...
		nrf_err = ble_hrs_client_handles_assign(hrs, evt->conn_handle, &evt->peer_db);
		if (nrf_err != 0) {
	...
	case BLE_HRS_CLIENT_EVT_HRM_NOTIFICATION:
		...
		nrf_err = ble_hrs_heart_rate_measurement_send(&ble_hrs, evt->hrm.hr_value);
		if (nrf_err) {
			...
			if (nrf_err != NRF_ERROR_NOT_FOUND && nrf_err != NRF_ERROR_INVALID_STATE) {
				LOG_ERR("Failed to update heart rate measurement, nrf_error %#x",
					nrf_err);
			}
		} else {
			LOG_INF("Sent heart rate to Central.");
		}
		break;
