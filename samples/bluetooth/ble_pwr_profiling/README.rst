...
LED 0:
  Lit when the device is initialized, if configured. (See LED note)

LED 1:
  Lit when a device is connected, if configured. (See LED note)
...
.. note::
   ...
   You can enable LEDs by setting the :kconfig:option:`CONFIG_APP_BLE_PWR_PROFILING_LED` Kconfig option.
   Enabling LEDs will increase the power consumption in active and low power mode by ~1.8 uA when not connected and by ~3.7 uA when connected to a device because of the 2nd LED light on.
