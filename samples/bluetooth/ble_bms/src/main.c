...
static void ble_adv_evt_handler(struct ble_adv *ble_adv, const struct ble_adv_evt *evt)
{
	...
	case BLE_ADV_EVT_WHITELIST_REQUEST:
		nrf_err = pm_whitelist_get(whitelist_addrs, &addr_cnt,
...
#if defined(CONFIG_USE_AUTHORIZATION_CODE)
		if ((evt->auth.auth_code.len != strlen(CONFIG_AUTHORIZATION_CODE)) ||
		    (memcmp(CONFIG_AUTHORIZATION_CODE, evt->auth.auth_code.code,
			    strlen(CONFIG_AUTHORIZATION_CODE)) != 0)) {
			is_authorized = false;
		}
#endif
