...
The file must be added to the build configuration in VS Code or as an extra argument to west: ``-DEXTRA_CONF_FILE="lpuarte.conf"``.
...
.. tabs::
   ...
   .. group-tab:: UARTE configuration
      ...
      Test the sample by using the standard UARTE interface with two devices, one running this sample (:ref:`ble_nus_central_sample`) and another running the :ref:`ble_nus_sample` sample. This is the default configuration.
   ...
   .. group-tab:: LPUARTE configuration
      ...
      #. Connect the pins on the device in loopback mode as described below, as defined in :file:`board-config.h`.
