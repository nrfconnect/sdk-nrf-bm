 ...
 * service module.
 */
static uint16_t current_rx_buf_len = BLE_NUS_CLIENT_MAX_DATA_LEN_CALC(BLE_GATT_ATT_MTU_DEFAULT);
...
static void lpuarte_rx_handler(char *data, size_t data_len)
{
	...
	do {
		nrf_err = ble_nus_client_string_send(&nus_c, data, len);
...
static void uarte_rx_handler(char *data, size_t data_len)
{
	...
	for (int i = 0; i < data_len; i++) {
		...
		if ((c == '\n' || c == '\r') || (rx_buf_idx >= current_rx_buf_len)) {
			...
			do {
				nrf_err = ble_nus_client_string_send(&nus_c, rx_buf, len);
...
static void scan_init(void)
{
	...
	err = ble_scan_init(&scan, &init_scan);
	if (err) {
	...
	err = ble_scan_filter_add(&scan, BLE_SCAN_UUID_FILTER, &uuid_filter);
	if (err) {
		LOG_ERR("nrf_ble_scan_filter_add uuid failed, nrf_error %#x", err);
	}
	...
	err = ble_scan_filter_add(&scan, BLE_SCAN_NAME_FILTER, &name_filter);
	if (err) {
		LOG_ERR("Failed to set filter, nrf_error %#x", err);
	}
#endif
	err = ble_scan_filters_enable(&scan, BLE_SCAN_NAME_FILTER | BLE_SCAN_UUID_FILTER, false);
	if (err) {
...
{
	ble_nus_client_on_db_disc_evt(&nus_c, evt);
}
				       ...
				       struct ble_nus_client_evt const *ble_nus_evt)
{
	...
	case BLE_NUS_CLIENT_EVT_DISCOVERY_COMPLETE:
		...
		break;
	case BLE_NUS_CLIENT_EVT_TX_EVT:
		LOG_INF("BLE_NUS_CLIENT_EVT_TX_EVT Data len: %d",
			ble_nus_evt->nus_tx_evt.data_len);
#if defined(CONFIG_SAMPLE_NUS_CENTRAL_LPUARTE)
		LOG_INF("BLE_NUS_CLIENT_EVT_TX_EVT event");
		err = bm_lpuarte_tx(&lpu, ble_nus_evt->nus_tx_evt.data,
				    ble_nus_evt->nus_tx_evt.data_len, 3000);
		if (err) {
			LOG_ERR("bm_lpuarte_tx failed, err %d", err);
		}
#else
		err = nrfx_uarte_tx(&nus_uarte_inst, ble_nus_evt->nus_tx_evt.data,
				    ble_nus_evt->nus_tx_evt.data_len, NRFX_UARTE_TX_BLOCKING);
		if (err) {
...
static void ble_evt_handler(ble_evt_t const *ble_evt, void *context)
{
	...
	case BLE_GAP_EVT_CONNECTED:
		err_code = ble_nus_client_handles_assign(&nus_c, ble_evt->evt.gap_evt.conn_handle,
							 NULL);
		...
		err_code = ble_db_discovery_start(&db_disc, ble_evt->evt.gap_evt.conn_handle);
		if (err_code) {
...
NRF_SDH_BLE_OBSERVER(sdh_ble, ble_evt_handler, NULL, USER_LOW);
...
static void ble_stack_init(void)
{
	...
	/** Enable BLE stack. */
	err_code = nrf_sdh_ble_enable(CONFIG_NRF_SDH_BLE_CONN_TAG);
	if (err_code) {
...
static void conn_params_evt_handler(const struct ble_conn_params_evt *evt)
{
	...
	case BLE_CONN_PARAMS_EVT_ATT_MTU_UPDATED:
		LOG_INF("GATT ATT MTU on connection 0x%x changed to %d", evt->conn_handle,
			evt->att_mtu);
		current_rx_buf_len = BLE_NUS_MAX_DATA_LEN_CALC(evt->att_mtu);
		break;
...
static void nus_client_init(void)
{
	...
	init.gatt_queue = &gatt_queue;
	init.db_discovery = &db_disc;

	err_code = ble_nus_client_init(&nus_c, &init);
	if (err_code) {
...
static void db_discovery_init(void)
{
	...
	db_cfg.gatt_queue = &gatt_queue;

	err_code = ble_db_discovery_init(&db_disc, &db_cfg);
