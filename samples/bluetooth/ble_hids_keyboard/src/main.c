...
int report_fifo_put(struct ble_hids_input_report *report)
{
	uint32_t written;
	uint32_t space;
	...
	if (space < (sizeof(struct ble_hids_input_report) + CONFIG_BLE_HIDS_INPUT_REPORT_MAX_LEN)) {
		LOG_ERR("Could not put input report in buffer");
		return NRF_ERROR_NO_MEM;
	}

	NRFX_CRITICAL_SECTION_ENTER();
	written = ring_buf_put(&report_fifo, (void *)report, sizeof(struct ble_hids_input_report));
