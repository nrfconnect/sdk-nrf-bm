...
add_custom_command(
  ...
  COMMAND ${CMAKE_COMMAND} -DZEPHYR_BASE=${ZEPHYR_BASE}
    ...
    # CMake instead.
    -DVERSION_FILE=${ZEPHYR_NRF_BM_MODULE_DIR}/VERSIONFILEDUMMY
    ...
    DEPENDS ${ZEPHYR_NRF_BM_MODULE_DIR}/VERSION ${git_dependency}
)
...
if(GIT_FOUND)
  # The gitdir folder will not be under ZEPHYR_NRF_BM_MODULE_DIR when using git worktrees.
  # Use git rev-parse to find the absolute path to the git directory.
  execute_process(
    COMMAND ${GIT_EXECUTABLE} rev-parse --absolute-git-dir
    WORKING_DIRECTORY                ${ZEPHYR_NRF_BM_MODULE_DIR}
  ...
  set(ABSOLUTE_GIT_DIR ${ZEPHYR_NRF_BM_MODULE_DIR}/".git")
endif()
...
add_custom_command(
  ...
  COMMAND ${CMAKE_COMMAND} -DZEPHYR_BASE=${ZEPHYR_BASE} -DNRF_DIR=${NRF_DIR}
    ...
    -DCOMMIT_PATH=${ZEPHYR_NRF_BM_MODULE_DIR}
    -P ${ZEPHYR_NRF_MODULE_DIR}/cmake/gen_commit_h.cmake
    DEPENDS ${ZEPHYR_NRF_BM_MODULE_DIR}/VERSION ${ABSOLUTE_GIT_DIR} ${ABSOLUTE_GIT_DIR}/index
)
