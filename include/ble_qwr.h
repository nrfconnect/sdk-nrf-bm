...
/**
 * @brief Macro for defining an array of ble_qwr instance.
 *
 * @param _name Name of the array.
 * @param _cnt Size of the array.
 * @hideinitializer
 */
#define BLE_QWRS_DEF(_name, _cnt)                                                                  \
	static struct ble_qwr _name[_cnt];                                                         \
 ...
 */
typedef uint16_t (*ble_qwr_evt_handler_t)(struct ble_qwr *qwr, struct ble_qwr_evt *evt);
...
struct ble_qwr {
	 ...
	 */
	ble_qwr_evt_handler_t event_handler;
	/** Connection handle. */
