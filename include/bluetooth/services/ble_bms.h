 ...
 * APP<-BMS      [label="BLE_BMS_EVT_AUTH {auth_code}"];
 * ---           [label="Variant #1: app grants authorization"];
 * APP->BMS      [label="ble_bms_auth_response (true)"];
 ...
 * APP->BMS      [label="ble_bms_auth_response (false)"];
 * BMS>>APP      [label="NRF_SUCCESS"];
...
NRF_SDH_BLE_OBSERVER(_name ## _obs,                                                                \
		     ...
		     CONFIG_BLE_BMS_BLE_OBSERVER_PRIO)
...
#define BLE_BMS_OPCODE_NOT_SUPPORTED (BLE_GATT_STATUS_ATTERR_APP_BEGIN + 0)
...
struct ble_bms_config {
	...
	struct ble_bms_bond_cbs bond_callbacks;
};
 ...
 *
 * @details This function should be called when receiving the @ref BLE_BMS_EVT_AUTH event to
