 ...
 *          the initialization of the Scanning Module. @ref BLE_SCAN_EVT_WHITELIST_REQUEST
 ...
 *          the connection will not be established with the first device found after this function
 * 			is called.
 *
 * @param[in] scan_ctx            Pointer to the Scanning Module instance.
 *
 ...
 *          BLE_SCAN_EVT_WHITELIST_ADV_REPORT event is generated. The generation of this event means
 *          that there is a risk that the whitelist is empty. In such case, this function can change
 *          the scanning parameters, so that the whitelist is not used, and you avoid the error
 * 			caused by scanning with the whitelist when there are no devices on the
 * 			whitelist.
 *
 * @param[in,out] scan_ctx     Pointer to the Scanning Module instance.
 ...
 *          displayed (for example, on a phone screen from left to right), you must use this
 * 			function to convert the address to the SoftDevice address type.
 *
