 ...
 *
 *          This module uses ZMS to interface with persistent storage.
 *          It uses a Devicetree storage partition exclusive to the Peer Manager.
 */
 ...
 *
 * @note You must initialize the Peer Manager before you can call any other Peer Manager
 *       function.
 *
...
/**
 * @brief Register an event handler with the Peer Manager.
 *
 * @param[in] event_handler  Callback for events from the @ref peer_manager module.
 *                           @p event_handler is called for every event that the Peer Manager sends
...
/**
 * @brief Provide pairing and bonding parameters to use for pairing procedures.
 *
...
/**
 * @brief Establish encryption on a connection, and optionally establish a bond.
 *
 * @details This function attempts to secure the link that is specified by @p conn_handle.
 ...
 *          If no key exists, the function attempts to perform pairing and bonding according to the
 *          currently active security parameters.
 *
 ...
 *                             encryption key. This argument is relevant only for the central role.
 ...
 *
 * @retval NRF_SUCCESS              If the reply was successful.
 * @retval NRF_ERROR_NULL           If @p sec_params or @p context were NULL.
 * @retval NRF_ERROR_INVALID_PARAM  If the value of @p sec_params was invalid.
 * @retval NRF_ERROR_INVALID_STATE  If the Peer Manager is not initialized.
 */
 ...
 * @retval NRF_SUCCESS              If pairing was initiated successfully.
 * @retval NRF_ERROR_INVALID_STATE  If the Peer Manager is not initialized.
 * @retval NRF_ERROR_FORBIDDEN      If LESC module support is enabled (see the @c CONFIG_PM_LESC
 *                                  Kconfig option).
 */
 ...
 *
 * @param[in] peers     The peers to add to the device identities list. Pass NULL to clear the
 *                      device identities list.
 ...
 * means that bonded devices might not recognize this device.
 *
 ...
 *                                         function was called while advertising, scanning, or while
 *                                         connected.
 * @retval NRF_ERROR_INTERNAL              If an internal error occurred.
 */
...
/**
 * @brief Configure privacy settings.
 *
 ...
 *
 * @retval true   If the IRK used matched the one used to create the address.
 ...
 *                          resolved. The @p conn_handle can refer to a recently disconnected
 *                          connection.
 *
 ...
 *          is the same as for @ref pm_next_peer_id_get. If the @p first_peer_id
 ...
 * @param[in]     skip_id        It determines which peer ID will be added to the list.
 *
 ...
 *
 * @details This function returns the number of peers for which there is data in non-volatile
 *          storage.
 *
...
/**
 * @brief Retrieve the stored data of a peer.
 *
 * @note The length of the provided buffer must be a multiple of 4 bytes.
 *
...
/**
 * @brief Set or update stored data of a peer.
 *
 * @note Writing the data to non-volatile storage happens asynchronously. Therefore, the buffer
 ...
 * @retval NRF_ERROR_BUSY           If the underlying storage handler is busy with other storage
 *                                  operations. Try again after receiving a Peer Manager event.
 * @retval NRF_ERROR_RESOURCES      If there is not enough space in non-volatile storage.
...
/**
 * @brief Delete specific data stored for a peer.
 *
 ...
 * @param[in] data_id  The data to clear.
 *
 ...
 *                                  successfully.
 * @retval NRF_ERROR_NULL           If @p bonding_data or @p new_peer_id are NULL.
 ...
 * @retval NRF_ERROR_BUSY           If the underlying storage is busy with other storage
 *                                  operations. Try again after receiving a Peer Manager event.
 * @retval NRF_ERROR_INVALID_STATE  If the Peer Manager is not initialized.
 * @retval NRF_ERROR_INTERNAL       If an internal error occurred.
 */
 ...
 *          deleted. If the peer is or becomes connected or data is manually written in storage
 *          during this procedure (until the success or failure event happens), the behavior
 *          is undefined.
 *
 ...
 *
 * @note If there is no peer data in storage the @ref PM_EVT_PEER_DELETE_SUCCEEDED event is sent
...
/**
 * @brief Find the highest and lowest ranked peers.
 *
 * @details The rank is saved in non-volatile storage under the data ID @ref
 ...
 *          PM_EVT_PEER_DATA_UPDATE_SUCCEEDED or a @ref PM_EVT_PEER_DATA_UPDATE_FAILED event is sent
 *          with a @ref PM_STORE_TOKEN_INVALID store token when the operation is complete.
 ...
 * @retval NRF_ERROR_INVALID_PARAM  If @p peer_id is invalid, or doesn't exist in non-volatile
 *                                  storage.
 * @retval NRF_ERROR_RESOURCES      If there is not enough space in non-volatile storage.
 * @retval NRF_ERROR_BUSY           If the underlying storage handler is busy with other storage
 ...
 * @retval NRF_ERROR_INVALID_STATE  If the Peer Manager is not initialized.
 * @retval NRF_ERROR_DATA_SIZE      If the highest rank is @ref UINT32_MAX, so the new rank would
 *                                  wrap around to 0. To fix this, manually update all ranks to
