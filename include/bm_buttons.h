...
/** @file
 *
 * @defgroup bm_buttons NCS Bare Metal Buttons library
 * @{
 *
 ...
 *        configuration.
 */
enum bm_buttons_pin_pull {
	/* Pin pull-up resistor disabled. */
	BM_BUTTONS_PIN_NOPULL = NRF_GPIO_PIN_NOPULL,
	/* Pin pull-down resistor enabled. */
	BM_BUTTONS_PIN_PULLDOWN = NRF_GPIO_PIN_PULLDOWN,
	/* Pin pull-up resistor enabled. */
	BM_BUTTONS_PIN_PULLUP = NRF_GPIO_PIN_PULLUP,
};
 ...
 *       @ref bm_buttons_deinit is called.
 *       If the @p configs configurations are altered or destroyed prematurely, it will result in
 *       undefined behavior.
 *
 * @note The @ref bm_buttons_enable function must be called in order to enable the button
 *       detection.
 *
